...
# and work just fine. Because the WAL summarizer was unwilling to do the same
# thing, it got stuck. This test aims to validate that this bug has been fixed.
...
# Set up node2 as a standby for node1.
...
# Set up node3 as a standby for node2. To get the desired behavior, we need a few
# configuration changes here. First, we must enable summarization on the new node,
# since that's what we want to test. We also set log_min_messages = debug1 since
# WAL summarization prints a bunch of diagnostics at that level that may be useful
# in case this test fails. Second, we must clear primary_conninfo, so that it can
# only obtain WAL from the archive. Finally, we lower wal_retrieve_retry_interval
# to avoid long, unnecessary waits while running the test.
...
# Record the WAL insert LSN on node1, so we can later verify that summarization on
# node3 advances past this point.
...
#
# In more detail: the WAL segment that contains the current insert LSN exists on
# timeline 1, but since all we did is CREATE TABLE dummy (), it wasn't full. We're
# now running on timeline 2, and pg_switch_wal() fills up the rest of the segment.

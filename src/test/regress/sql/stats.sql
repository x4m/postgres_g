...
-- (here, building a B-tree index) are tracked in pg_stat_io in the bulkload
-- IOContext.
SELECT sum(extends) AS io_sum_bulkload_extends_before
  FROM pg_stat_io WHERE context = 'bulkload' AND object = 'relation' \gset
CREATE INDEX test_io_bulkwrite_strategy_idx
  ON test_io_bulkwrite_strategy (i);
SELECT pg_stat_force_next_flush();
SELECT sum(extends) AS io_sum_bulkload_extends_after
  FROM pg_stat_io WHERE context = 'bulkload' AND object = 'relation' \gset
SELECT :io_sum_bulkload_extends_after > :io_sum_bulkload_extends_before;

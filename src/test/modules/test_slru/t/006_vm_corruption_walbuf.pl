...
#   3. A checkpoint, already past its checkpoint WAL record and waiting just
#      before CheckPointBuffers(), is released.  It later flushes the dirty heap
#      and VM pages through the real checkpointer path.
...
# Park the checkpointer after it has inserted the checkpoint redo record but
# before it scans shared buffers.  Once p1 dirties heap/VM pages and stalls in
# WAL insertion, waking this checkpoint makes the actual checkpointer write the
# pages.
$node->safe_psql('postgres',
	q{SELECT injection_points_attach('checkpoint-before-buffer-sync', 'wait')});
my $waker = $node->background_psql('postgres', on_error_stop => 0);
$waker->query_safe(
	q{PREPARE wake_checkpoint AS SELECT injection_points_wakeup('checkpoint-before-buffer-sync')});
my $ckpt = $node->background_psql('postgres', on_error_stop => 0);
$ckpt->query_until(
	qr/ckpt_go/, q{
	\echo ckpt_go
	CHECKPOINT;
});
$node->wait_for_event('checkpointer', 'checkpoint-before-buffer-sync');
...
# Let the checkpointer continue.  It can only flush the dirty heap page after
# p1 exits on postmaster death and releases its locks.
$waker->query_safe(q{EXECUTE wake_checkpoint});
usleep(200_000);

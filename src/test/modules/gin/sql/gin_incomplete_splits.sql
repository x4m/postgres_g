...
-- Verify that bitmap scan finds every remaining row
explain (costs off)
select count(*) as onecount from gin_posting_tree where i @> '{1}';
select count(*) as onecount from gin_posting_tree where i @> '{1}';

-- Verify that a sequential scan gives the same result

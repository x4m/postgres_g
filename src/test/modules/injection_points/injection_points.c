...
injection_map_state(InjectionMapMode mode, int elevel)
{
	...
	if (mode == INJ_MAP_CREATE)
		(void) unlink(INJ_STATE_FILE);	/* drop any stale file from a crash */
...
#ifndef WIN32
	{
		...
		fd = OpenTransientFile(INJ_STATE_FILE, oflags);
...
#else
	{
		...
		hfile = CreateFile(INJ_STATE_FILE,

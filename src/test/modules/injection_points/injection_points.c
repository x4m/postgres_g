...
injection_wait(const char *name, const void *private_data, void *arg)
{
	...
	{
		/*
		 * If this point was armed through the filesystem, publish our presence
		 * there so an out-of-process test can see that we are parked here and
		 * release us with no SQL connection.  The point directory must have
		 * been created by the controller: a point attached only through SQL
		 * must not leave a directory that would re-arm it after a crash
		 * restart.
		 */

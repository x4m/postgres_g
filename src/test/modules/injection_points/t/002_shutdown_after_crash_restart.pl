...
# point is re-armed after the crash, and the parked process is released by
# removing its PID file.
...
EOF
	...
	# Arm the wait point before the server starts, purely through the
	# filesystem.  injection_points rescans this directory whenever shared
	# memory is (re)created, so the point is re-attached by the replacement
	# startup process after the crash restart, when no SQL connection exists
	# and shared memory has been reset.

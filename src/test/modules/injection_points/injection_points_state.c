 ...
 *
 *	  - arm/disarm a "wait" injection point by writing the core registry file
 ...
 * intended use (arming points out of band, before or alongside controlled
 * test sessions).
 *
 * Usage:
 ...
 * so we can read and write the same bytes to arm and disarm points.  The field
 * sizes below must match the backend's INJ_*_MAXLEN / MAX_INJECTION_POINTS.
 */
...
{
	uint64_t	generation;		/* even: free, odd: in use */
...
/*
 * Arm a "wait" injection point by adding it to the registry, mimicking
...
/* Disarm an injection point by flipping its generation back to even. */
static int

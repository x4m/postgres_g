...
# Advance the logical slot to the current WAL position so that after the
# crash its restart_lsn is recent.  Without this the post-recovery
# pg_logical_slot_get_changes call has to scan all the WAL generated by
# the advance_wal() calls above, which is slow on Windows.
$node->safe_psql('postgres',
	q{select count(*) from pg_logical_slot_get_changes('slot_logical', null, null)});
...
my $slot_lsn_before = $node->safe_psql('postgres',
	q{select restart_lsn from pg_replication_slots where slot_name = 'slot_logical'});
note("slot restart_lsn before checkpoint resumes: $slot_lsn_before");
$node->safe_psql('postgres',
	q{select pg_replication_slot_advance('slot_physical', pg_current_wal_lsn())}
);
...
# Check the slot's restart_lsn so that any failure has actionable context.
my $restart_lsn = $node->safe_psql('postgres',
	q{select restart_lsn from pg_replication_slots where slot_name = 'slot_logical'});
note("slot restart_lsn after recovery: $restart_lsn");

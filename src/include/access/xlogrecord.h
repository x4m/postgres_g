...
typedef struct XLogRecord
{
	...
	/*
	 * XLogRecordBlockHeaders, XLogRecordDataHeader or compression header
	 * follow, no padding
	 */
...
/*
 * Whole-record compression is only offered for methods that earn it, so a
 * build without any of them carries neither side of the machinery.  It must
 * still recognize such a record when reading, to report it properly.
 */
#if defined(USE_LZ4) || defined(USE_ZSTD)
#define WAL_WHOLE_RECORD_COMPRESSION 1
#endif

/*
 * Header prepended to a whole-record compressed WAL record.
 *
 * The compressed payload follows immediately, at SizeOfXLogCompressedRecord.
 * record_header.xl_tot_len covers this header plus that payload, so it is the
 * on-disk size; decompressed_length is what the record expands back to,
 * including its XLogRecord header.
 */

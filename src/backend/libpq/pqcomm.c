...
pq_get_compressed_message(StringInfo s)
{
	...
	if (!PqDecompressionBufferInitialized)
	{
		...
		initStringInfo(&PqDecompressionBuffer);
	...
	for (;;)
	{
		...
		if (pq_getmessage(&compressed, PQ_COMPRESSION_MAX_WRAPPER_SIZE))
			ereport(FATAL,
					(errcode(ERRCODE_PROTOCOL_VIOLATION),
		...
		for (;;)
		{
			...
			enlargeStringInfo(&PqDecompressionBuffer, output_size);
		...
		if (PqDecompressionBuffer.len >= 5)
		{
			...
			if (message_length + 1 == PqDecompressionBuffer.len)
			{
				if (result == 0)
					goto invalid_contents;
				resetStringInfo(s);
				appendBinaryStringInfo(s, PqDecompressionBuffer.data + 5,
...
socket_compression_init(void)
{
	...
	initStringInfo(&PqCompressionOutput);
	MemoryContextSwitchTo(oldcontext);

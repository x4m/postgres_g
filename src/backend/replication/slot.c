 ...
 *
 * The caller supplies oldest xmin along with the oldest slot xmin and oldest
 * catalog xmin, all obtained from the same ComputeXidHorizons() call. We use them
 * to avoid unnecessary work: if a replication slot is not what's holding back
 * OldestXmin (e.g., a long-running transaction is), invalidating slots would
 * not help.  Even when a slot is holding back OldestXmin, we still skip the
 * invalidation call if OldestXmin has not yet exceeded max_slot_xid_age.
 *
							...
							TransactionId OldestSlotCatalogXmin)
{
	 ...
	 */
	slotHoldsOldestXminBack =
		(TransactionIdIsValid(OldestSlotXmin) &&
		 TransactionIdEquals(OldestXmin, OldestSlotXmin)) ||
		(TransactionIdIsValid(OldestSlotCatalogXmin) &&
		 TransactionIdEquals(OldestXmin, OldestSlotCatalogXmin));

	if (!slotHoldsOldestXminBack)
		return false;

	xidLimit = TransactionIdRetreatedBy(ReadNextTransactionId(),
										max_slot_xid_age);

	/*
	 * A replication slot is holding back OldestXmin. Now check whether the
	 * slot holding it back has also exceeded the XID age.
	 */
	if (TransactionIdPrecedes(OldestXmin, xidLimit))

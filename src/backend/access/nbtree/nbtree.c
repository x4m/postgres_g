...
backtrack:
	...
	else if (P_ISLEAF(opaque))
	{
		 ...
		 *
		 * We also attempt page deletion if the page is 95% empty (by bytes).
		...
		{
			/* Check if page is 95% empty by space usage */
			 ...
			 */
			if (freespace >= (pagesize * 95) / 100 && ndeletable == 0 && nupdatable == 0)
				attempt_pagedel = true;
		}
		 ...
		 * we're attempting a merge of a 95% empty page with tuples.
		 */
		Assert(!attempt_pagedel || nhtidslive == 0 ||
			   (blkno == scanblkno && PageGetFreeSpace(page) >= ((BLCKSZ - SizeOfPageHeaderData) * 95) / 100));
	}

...
_bt_pagedel(Relation rel, Buffer leafbuf, BTVacState *vstate)
{
	...
	for (;;)
	{
		...
		if (page_has_tuples)
		{
			...
			if (rightsib != P_NONE)
			{
				rbuf = _bt_getbuf(rel, rightsib, BT_READ);
				...
				else
				{
					elog(DEBUG1, "Page merge not feasible for index \"%s\": right sibling page %u is not suitable (is_leaf=%d, deleted=%d, halfdead=%d, level=%d vs %d)",
						 RelationGetRelationName(rel), rightsib, P_ISLEAF(ropaque), 
					   ...
					   BTStack stack)
{
	 ...
	 * empty. For merge scenarios (95% empty pages), page may have tuples.
	 */
	...
	Assert(P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page) || 
		   (P_FIRSTDATAKEY(opaque) <= PageGetMaxOffsetNumber(page) &&
		    PageGetFreeSpace(page) >= ((BLCKSZ - SizeOfPageHeaderData) * 95) / 100));
	Assert(heaprel != NULL);
						 ...
						 bool *rightsib_empty, BTVacState *vstate)
{
	...
	if (target == leafblkno)
	{
		...
		if (P_FIRSTDATAKEY(opaque) <= PageGetMaxOffsetNumber(page))
		{
			...
			if (PageGetFreeSpace(page) < ((BLCKSZ - SizeOfPageHeaderData) * 95) / 100)
				elog(ERROR, "target leaf page changed status unexpectedly in block %u of index \"%s\"",
					 target, RelationGetRelationName(rel));
		}
	...
	*rightsib_empty = (P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page));
	...
	if (target == leafblkno)
	{
		...
		if (minoff <= maxoff && P_ISLEAF(leafopaque) && P_ISHALFDEAD(leafopaque))
		{
			...
			for (i = minoff; i <= maxoff; i++)
			{
				ItemId		itemid = PageGetItemId(leafpage, i);
				space_needed += ItemIdGetLength(itemid) + sizeof(ItemIdData);
			}
				...
				PageGetFreeSpace(rightpage) >= space_needed)
			{
				...
				{
					ItemId		itemid = PageGetItemId(leafpage, i);
					IndexTuple	tuple = (IndexTuple) PageGetItem(leafpage, itemid);
	...
	if (RelationNeedsWAL(rel))
	{
		...
		/* information about merged tuples */
		xlrec.merge_ntuples = do_merge ? merge_ntuples : 0;

		XLogRegisterData(&xlrec, SizeOfBtreeUnlinkPage);

		/* Add merged tuple data to WAL record if merge occurred */
		if (do_merge && merge_ntuples > 0)
		{
			for (int i = 0; i < merge_ntuples; i++)
			{
				XLogRegisterData(merge_tuples[i], merge_tupsz[i]);
			}
		}

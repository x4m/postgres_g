				...
				bool is_build)
{
	...
	*splitinfo = NIL;
	...
	/*
	 * Derived metadata must not force a page split when the updated ordinary
	 * tuple vector still fits.  The non-split path will rebuild the groups, or
	 * omit them if their extra space is not available.
	 */
	if (is_split && has_skip)
	{
		IndexTuple *pagevec;
		int			pagelen;
		int			dst = 0;
		...
		pagevec = gistextractpage(page, &pagelen);
		for (int src = 0; src < pagelen; src++)
		{
			OffsetNumber off = src + FirstOffsetNumber;

			if (GistTupleIsSkip(pagevec[src]) || off == oldoffnum)
				continue;
			pagevec[dst++] = pagevec[src];
		}
		pagelen = dst;
		pagevec = gistjoinvector(pagevec, &pagelen, itup, ntup);
		for (int i = 0; i < pagelen; i++)
			required += IndexTupleSize(pagevec[i]) + sizeof(ItemIdData);
		if (required <= GiSTPageSize)
			is_split = false;
	}
	...
	else
	{
		...
		if (has_skip)
		{
			...
			if (pagelen > 0 &&
				gistFormSkipGroups(rel, page, pagevec, pagelen, giststate,
								   &newlist, &newlenlist, &newlen))
			{
				char	   *data = (char *) newlist;

				writetup = palloc_array(IndexTuple, newlen);
			...
			deloffs = palloc_array(OffsetNumber, maxoff);
			for (OffsetNumber off = FirstOffsetNumber; off <= maxoff;
				 off = OffsetNumberNext(off))
				deloffs[ndeloffs++] = off;
		}

		/* A page rewrite registers one WAL data segment per new tuple. */
		if (has_skip && !is_build && RelationNeedsWAL(rel))

...
int leafExclusiveLocks;
int scanToDeleteExclusiveLocks;
...
ginVacuumPostingTree(GinVacuumState *gvs, BlockNumber rootBlkno)
{
	...
	leafExclusiveLocks = 0;
	scanToDeleteExclusiveLocks = 0;
	elog(NOTICE,"Vacuum of posting tree at %d", rootBlkno);
	ginVacuumPostingTreeLeaves(gvs, rootBlkno, TRUE);
	elog(NOTICE,"Initial cleanup locked %d leaf pages exclusively",leafExclusiveLocks);
	elog(NOTICE,"Page removal scan locked %d pages exclusively",scanToDeleteExclusiveLocks);
}

				   ...
				   int nmembers, MultiXactMember *members)
{
	 ...
	 * it now.
	 *
	 * When test_slru.simulate_multixact_wrong_latest_page is enabled, skip
	 * pre-initialization to simulate the bug (wrong latest_page_number causes
	 * "read too few bytes" when crossing the page boundary).
	 */
	{
		const char *val = GetConfigOption("test_slru.simulate_multixact_wrong_latest_page", true, false);
		bool		simulate_wrong = (val != NULL && strcmp(val, "on") == 0);
	 ...
	 * On the primary, skip when crossing a page boundary to simulate pre-8ba61bc
	 * WAL order (ZERO_OFF_PAGE for next page comes after this CREATE_ID).
	 */
	if (next_pageno == pageno || InRecovery)
	{
		...
		else
		{
			...
			elog(DEBUG1, "RecordNewMultiXact: accessing next offsets page " INT64_FORMAT " for multixid %u",
				 next_pageno, multi);
...
GetNewMultiXactId(int nmembers, MultiXactOffset *offset)
{
	...
	/*
	 * Make sure there is room for this MXID in the file.  Simulate pre-8ba61bc
	 * WAL order (CREATE_ID before ZERO_OFF_PAGE for next page) for bug testing.
	 */
...
multixact_redo(XLogReaderState *record)
{
	...
	if (info == XLOG_MULTIXACT_ZERO_OFF_PAGE)
	{
		...
		elog(DEBUG1, "replaying multixact ZERO_OFF_PAGE: pageno " INT64_FORMAT, pageno);
	...
	else if (info == XLOG_MULTIXACT_CREATE_ID)
	{
		...
		elog(DEBUG1, "replaying multixact CREATE_ID: mid %u offset %u nmembers %u",
	...
	else if (info == XLOG_MULTIXACT_TRUNCATE_ID)
	{
		 ...
		 * SimpleLruTruncate.
		 *
		 * When test_slru.simulate_multixact_wrong_latest_page is enabled,
		 * use 0 instead of the correct page to simulate the bug where
		 * pre-initialization is skipped, causing "read too few bytes" when

...
/*
 * Maximum size of buffers used for whole-record compression. This limits
 * memory usage per backend. Records larger than this will not be compressed
 * as a whole (FPI compression still applies). 4MB should be sufficient for
 * most use cases while preventing excessive memory consumption.
 */
#define WAL_COMPRESSION_MAX_BUFFER_SIZE		(4 * 1024 * 1024)
 ...
 * size would exceed WAL_COMPRESSION_MAX_BUFFER_SIZE, we mark compression as
...
void XLogEnsureCompressionBuffer(uint32 extraLen)
{
	...
	/*
	 * Check if the record would exceed our buffer size limit. If so, skip
	 * whole-record compression for this record. FPI compression will still
	 * work since it uses per-block buffers.
	 */
	if (desired_buffer_size > WAL_COMPRESSION_MAX_BUFFER_SIZE)
	{
		compression_buffer_current_size = -1;
		return;
	}

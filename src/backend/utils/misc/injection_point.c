...
#ifdef EXEC_BACKEND
/*
 * Re-map the active injection points file early in an EXEC_BACKEND child.
 *
 * Forked children inherit the postmaster's mapping, but EXEC_BACKEND children
 * must map the file themselves.  attach_fn does that, but only once the child
 * reaches InitProcess(); that is too late for points that fire earlier in
 * startup (e.g. "backend-initialize", before the startup packet is read).
 * SubPostmasterMain() calls this beforehand so ActiveInjectionPoints is valid
 * throughout child startup.  The map is idempotent, so the later attach_fn
 * call simply finds it already mapped.
 */
void
InjectionPointShmemAttach(void)
{
	injection_map_points(INJ_MAP_ATTACH, FATAL);
}
#endif							/* EXEC_BACKEND */
...
InjectionPointCacheRefresh(const char *name)
{
	...
	/*
	 * The registry may not be mapped: in an EXEC_BACKEND child this runs
	 * before the attach callback has re-mapped the file, and a process that
	 * does not attach shared memory at all never maps it.  Either way there
	 * can be no active injection points, so return rather than dereferencing a
	 * NULL pointer.
	 */
	if (ActiveInjectionPoints == NULL)
		return NULL;

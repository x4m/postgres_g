...
% первичные величины — счётчики, времена медианами трёх прогонов с чередованием
% конфигураций.
%
% 1. Необязательные функции преобразования (d3a4f89d8a3^ / d3a4f89d8a3):
%    время точечного запроса и время построения для класса операторов с
%    тождественными compress/decompress. Ожидание: эффект в c_c, то есть виден
%    на запросах по индексу, целиком помещающемуся в память, и растёт с
%    ветвистостью. Проверить также, что сканирование без обращения к таблице
%    стало возможно без fetch.
% 2. Покрывающие атрибуты (f2e403803fe^ / f2e403803fe): ветвистость внутренних
%    уровней, высота, объём индекса и время запроса для трёх вариантов —
%    столбец в ключе, столбец покрывающий, столбца нет вовсе. Главная величина —
%    высота и ветвистость внутренних узлов: именно на них строится довод.
% 3. Перезапись элемента на месте (b1328d78f88^ / b1328d78f88): время вставки
%    отсортированных и случайных данных, объём индекса. Перепроверка чисел
%    2017 года на актуальном выпуске (заявлено ~3x и ~15%).

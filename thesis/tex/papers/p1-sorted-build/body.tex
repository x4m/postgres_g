...
\paragraph{Single run per configuration.} Ratios proved far more stable than
absolute values: three repetitions of the $10^6$ configurations varied by under
3\,\% in build time and under 5\,\% in accesses per query, while an earlier
measurement in which two jobs overlapped on the same machine gave build times
differing by 20\,\% for the same configuration. All numbers reported here come
from sequential runs; we nonetheless treat differences below roughly 10\,\% as
not established.

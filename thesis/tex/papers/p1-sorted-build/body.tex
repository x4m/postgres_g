...
\paragraph{Packing R-trees.} Roussopoulos and Leifker \cite{roussopoulos1985direct}
build a packed R-tree by sorting rectangles on a coordinate and filling nodes
consecutively. Kamel and Faloutsos \cite{kamel1993hilbert} sort on the Hilbert
...
space into slabs. All three are formulated in terms of coordinates or of the
area of a bounding rectangle. An operator class over signatures or over ranges
provides neither, so none of the three transfers to the framework setting. Our
contribution is not a better packing rule but the observation that the packing
rule can be \emph{delegated}: the order to the operator class, the split points
to \code{picksplit}.
...
optimise the tree built by insertion. An attempt by the present authors to
improve the penalty function of a generalized search tree along the same lines
...
attacks the CPU cost of scanning a node. Both are complementary to this work.
...
of the index. In our first version the comparator was supplied as a relation
option, which allowed any function of matching signature to be named~--- that
...
The procedure of Section~\ref{sec:build} fills a page to the fill factor and
...
Table~\ref{tab:levels} gives $\omega_l$ per level for ten million uniform
points.
...
Relative to insertion, occupancy-based packing builds 6.8 and 8.0 times faster
and makes queries 2.53 and 2.36 times more expensive.
\code{picksplit}-based partitioning builds 3.0 and 3.5 times faster and stays
...
Varying the degree of clustering (cluster sizes from $0.5f$ to $8f$, $N=10^6$)
leaves the ratio between the two sorted variants in the range 2.1--2.8 with no
systematic dependence on cluster size; we had expected a resonance with page
capacity and did not find one.
...
\midrule
occupancy & 10.01 & 5.57 & 15.20 & 10.17 \\
\code{picksplit} & 4.62 & 4.11 & 7.91 & 8.27 \\
\bottomrule
\end{tabular}
\end{table}
...
Build times for the Hilbert variant are not comparable with Z-order: our
experimental comparator does not implement abbreviated keys, so the difference
...
$k=4$, is conservative: $k=8$ improves query cost by 12\,\% at no cost in build
time, and $k=16$ by 17\,\% for 16\,\% more build time. Index size is independent
of $k$ from $k=2$ on. Returns diminish sharply after $k=4$, which is consistent
...
\midrule
insertion & 1\,134.3 & 1\,285\,510 & 23.66 & 40.95 \\
sorted, occupancy & 313.6 & 836\,842 & 15.20 & 61.01 \\
sorted, \code{picksplit} & 388.5 & 1\,128\,374 & 7.91 & 48.23 \\
\bottomrule
\end{tabular}
\end{table}
...
The ratio between the two sorted variants, 1.92, agrees with the 2.0--2.2
...
\code{picksplit} costs 130\,\% more than occupancy-based packing at $10^7$ and
24\,\% more at $1.38\times10^8$, as I/O takes over an increasing share of build
time.

\subsection{An unexpected result}
\label{sec:eval-surprise}

On this dataset \emph{insertion produces the worst tree of the three}: 23.66
accesses per selective query against 15.20 and 7.91, while taking three times as
long to build as \code{picksplit} and producing the largest index. On uniform
data the ordering was the opposite, with insertion nearly optimal
...
result: it rests on a single dataset and a single run. If it holds, it changes
...
With the index resident in the buffer cache, a spatial join performing several
hundred thousand descents spends 22.4\,\% of backend CPU in the loop over
entries of a page, 14.1\,\% in the operator class predicate, 12.8\,\% in the
buffer lookup table, 5.7\,\% in pinning buffers and 4.7\,\% in the function-call
machinery that invokes the predicate.

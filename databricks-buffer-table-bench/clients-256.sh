...
run_one() {
	...
	"$prefix/bin/pg_ctl" -D "$active_datadir" -l "$root/server.log" \
		-o "-p $port -c shared_buffers=128MB -c fsync=off -c synchronous_commit=off -c autovacuum=off -c max_connections=300 -c huge_pages=off -c checkpoint_timeout=1h -c max_wal_size=20GB" \
		start -w 9>&- >/dev/null
	...
	"$prefix/bin/pgbench" -p "$port" -n $flags -M prepared \
		--random-seed=104729 -c 256 -j 64 -T 8 bench 9>&- >/dev/null
	echo "RESULT workload=$workload clients=256 round=$round variant=$variant"
	"$prefix/bin/pgbench" -p "$port" -n $flags -M prepared \
		--random-seed=15485863 -c 256 -j 64 -T 30 bench 9>&- | grep "tps ="
...
run_pair() {
	...
	if [ $((round % 2)) -eq 1 ]; then
		run_one base "$workload" "$round"
		run_one patch "$workload" "$round"
	else
		run_one patch "$workload" "$round"
		run_one base "$workload" "$round"
	fi

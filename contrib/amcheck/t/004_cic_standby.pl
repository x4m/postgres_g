...
my $pgbench_h     = $node_standby_1->background_pgbench(
	'--no-vacuum --client=1 --transactions=2000',
	{
		'004_pgbench_standby_check_1' => q(
			SELECT bt_index_check('idx',true);
		   ),
        '004_pgbench_standby_check_2' => q(
			SELECT bt_index_check('idx',false);
		   )
	},
	\$pgbench_out,
	$pgbench_timer);
...
$node_primary->pgbench(
	'--no-vacuum --client=5 --transactions=2000',
	0,
	...
	'concurrent INSERTs and CIC',
	{
		...
		'004_pgbench_concurrent_transaction_savepoints' => q(
			BEGIN;
			SAVEPOINT s1;
			INSERT INTO tbl VALUES(0);
			COMMIT;

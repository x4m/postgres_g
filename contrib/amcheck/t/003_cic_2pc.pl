...
#
# Run 3 overlapping 2PC transactions with concurrent reindex rebuild
#
# We have two concurrent background psql processes: $main_h for INSERTs and
# $reindex_h for cuncurrent reindex.  Also, we use non-background psql for
# some COMMIT PREPARED statements.
#
...
my $main_h =
  $node->background_psql('postgres', \$main_in, \$main_out, $main_timer,
	on_error_stop => 1);
$main_in .= q(
BEGIN;
...
$reindex_in    = '';
$reindex_out   = '';
$reindex_timer = IPC::Run::timeout(180);
$reindex_h =
  $node->background_psql('postgres', \$reindex_in, \$reindex_out,
	$reindex_timer, on_error_stop => 1);
$reindex_in .= q(

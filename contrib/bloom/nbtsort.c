...
static double _xt_spools_heapscan(Relation heap, Relation index,
								  BTBuildState *buildstate, IndexInfo *indexInfo);
static void _xt_spooldestroy(BTSpool *btspool);
static void _xt_spool(BTSpool *btspool, ItemPointer self,
					  Datum *values, bool *isnull);
static void _xt_leafbuild(BTSpool *btspool, BTSpool *btspool2);
static void _xt_build_callback(Relation index, ItemPointer tid, Datum *values,
							   bool *isnull, bool tupleIsAlive, void *state);
static BulkWriteBuffer _xt_blnewpage(BTWriteState *wstate, uint32 level);
static BTPageState *_xt_pagestate(BTWriteState *wstate, uint32 level);
static void _xt_slideleft(Page rightmostpage);
static void _xt_sortaddtup(Page page, Size itemsize,
						   IndexTuple itup, OffsetNumber itup_off,
						   bool newfirstdataitem);
static void _xt_buildadd(BTWriteState *wstate, BTPageState *state,
						 IndexTuple itup, Size truncextra);
static void _xt_sort_dedup_finish_pending(BTWriteState *wstate,
										  BTPageState *state,
										  BTDedupState dstate);
static void _xt_uppershutdown(BTWriteState *wstate, BTPageState *state);
static void _xt_load(BTWriteState *wstate,
					 BTSpool *btspool, BTSpool *btspool2);
static void _xt_begin_parallel(BTBuildState *buildstate, bool isconcurrent,
							   int request);
static void _xt_end_parallel(BTLeader *btleader);
static Size _xt_parallel_estimate_shared(Relation heap, Snapshot snapshot);
static double _xt_parallel_heapscan(BTBuildState *buildstate,
									bool *brokenhotchain);
static void _xt_leader_participate_as_worker(BTBuildState *buildstate);
static void _xt_parallel_scan_and_sort(BTSpool *btspool, BTSpool *btspool2,
 ...
 */
IndexBuildResult * _xtbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	...
	reltuples = _xt_spools_heapscan(heap, index, &buildstate, indexInfo);
	 ...
	 */
	_xt_leafbuild(buildstate.spool, buildstate.spool2);
	_xt_spooldestroy(buildstate.spool);
	if (buildstate.spool2)
		_xt_spooldestroy(buildstate.spool2);
	if (buildstate.btleader)
		_xt_end_parallel(buildstate.btleader);
 ...
 * call _xt_end_parallel() in parallel case after it is done with spool/spool2.
 *
 * Returns the total number of heap tuples scanned.
 */
static double
_xt_spools_heapscan(Relation heap, Relation index, BTBuildState *buildstate,
					IndexInfo *indexInfo)
{
	...
	if (indexInfo->ii_ParallelWorkers > 0)
		_xt_begin_parallel(buildstate, indexInfo->ii_Concurrent,
	...
	if (!buildstate->btleader)
		reltuples = table_index_build_scan(heap, index, indexInfo, true, true,
										   _xt_build_callback, (void *) buildstate,
										   NULL);
	else
		reltuples = _xt_parallel_heapscan(buildstate,
 ...
 */
static void
_xt_spooldestroy(BTSpool *btspool)
{
 ...
 */
static void
_xt_spool(BTSpool *btspool, ItemPointer self, Datum *values, bool *isnull)
{
...
/*
 * given a spool loaded by successive calls to _xt_spool,
 * create an entire btree.
 */
static void
_xt_leafbuild(BTSpool *btspool, BTSpool *btspool2)
{
	BTWriteState wstate;
	...
	wstate.inskey = _xt_mkscankey(wstate.index, NULL);
	/* _xt_mkscankey() won't set allequalimage without metapage */
	wstate.inskey->allequalimage = _xt_allequalimage(wstate.index, true);
	...
	_xt_load(&wstate, btspool, btspool2);
}
 ...
 */
static void
_xt_build_callback(Relation index,
				   ItemPointer tid,
				   Datum *values,
				   bool *isnull,
				   bool tupleIsAlive,
				   void *state)
{
	...
	if (tupleIsAlive || buildstate->spool2 == NULL)
		_xt_spool(buildstate->spool, tid, values, isnull);
	else
	{
		/* dead tuples are put into spool2 */
		buildstate->havedead = true;
		_xt_spool(buildstate->spool2, tid, values, isnull);
	}
...
_xt_blnewpage(BTWriteState *wstate, uint32 level)
{
	BulkWriteBuffer buf;
	Page		page;
	BTPageOpaque opaque;
 ...
 */
static void
_xt_blwritepage(BTWriteState *wstate, BulkWriteBuffer buf, BlockNumber blkno)
{
 ...
 * is suitable for immediate use by _xt_buildadd.
 */
static BTPageState *
_xt_pagestate(BTWriteState *wstate, uint32 level)
{
	BTPageState *state = (BTPageState *) palloc0(sizeof(BTPageState));

	/* create initial page for level */
	state->btps_buf = _xt_blnewpage(wstate, level);
 ...
 *
 * _xt_blnewpage() makes the P_HIKEY line pointer appear allocated, but the
 ...
 */
static void
_xt_slideleft(Page rightmostpage)
{
	OffsetNumber off;
	OffsetNumber maxoff;
	ItemId		previi;
 ...
 *
 * This is very similar to nbtinsert.c's _xt_pgaddtup(), but this variant
 ...
 * calling _xt_slideleft().
 */
static void
_xt_sortaddtup(Page page,
			   Size itemsize,
			   IndexTuple itup,
 ...
 */
static void
_xt_buildadd(BTWriteState *wstate, BTPageState *state, IndexTuple itup,
			 Size truncextra)
{
	 ...
	 * a heap TID appended within _xt_truncate().  That may leave a new pivot
	...
	if (unlikely(itupsz > BTMaxItemSize(npage)))
		_xt_check_third_page(wstate->index, wstate->heap, isleaf, npage,
							 itup);
	 ...
	 * _xt_check_third_page() rejecting oversized non-pivot tuples.  On
		...
		(pgspc + last_truncextra < state->btps_full && last_off > P_FIRSTKEY))
	{
		...
		nbuf = _xt_blnewpage(wstate, state->btps_level);
		npage = (Page) nbuf;
		...
		_xt_sortaddtup(npage, ItemIdGetLength(ii), oitup, P_FIRSTKEY,
					   !isleaf);

		/*
		 * Move 'last' into the high key position on opage.  _xt_blnewpage()
		...
		*hii = *ii;
		...
		if (isleaf)
		{
			 ...
			 * likely that _xt_truncate() can truncate away more attributes,
			 * whereas the split point used within _xt_split() is chosen much
			 * more delicately.  Even still, the lastleft and firstright
			 * tuples passed to _xt_truncate() here are at least not fully
			 ...
			 * between two unequal tuples, _xt_truncate() will avoid including
			...
			truncated = _xt_truncate(wstate->index, lastleft, oitup,
									 wstate->inskey);
		...
		if (state->btps_next == NULL)
			state->btps_next = _xt_pagestate(wstate, state->btps_level + 1);
		...
		_xt_buildadd(wstate, state->btps_next, state->btps_lowkey, 0);
		...
		/*
		 * Write out the old page. _xt_blwritepage takes ownership of the
		 * 'opage' buffer.
		 */
		_xt_blwritepage(wstate, obuf, oblkno);
	...
	_xt_sortaddtup(npage, itupsz, itup, last_off,
 ...
 *
 * This is almost like _xt_dedup_finish_pending(), but it adds a new tuple
 * using _xt_buildadd().
 */
static void
_xt_sort_dedup_finish_pending(BTWriteState *wstate, BTPageState *state,
							  BTDedupState dstate)
{
	Assert(dstate->nitems > 0);

	if (dstate->nitems == 1)
		_xt_buildadd(wstate, state, dstate->base, 0);
	else
	{
		...
		postingtuple = _xt_form_posting(dstate->base,
										dstate->htids,
										dstate->nhtids);
		...
		_xt_buildadd(wstate, state, postingtuple, truncextra);
		pfree(postingtuple);
	}
 ...
 */
static void
_xt_uppershutdown(BTWriteState *wstate, BTPageState *state)
{
	BTPageState *s;
	...
	for (s = state; s != NULL; s = s->btps_next)
	{
		...
		else
		{
			...
			_xt_buildadd(wstate, s->btps_next, s->btps_lowkey, 0);
			pfree(s->btps_lowkey);
			s->btps_lowkey = NULL;
		}
		 ...
		 */
		_xt_slideleft((Page) s->btps_buf);
		_xt_blwritepage(wstate, s->btps_buf, s->btps_blkno);
	...
	_xt_initmetapage((Page) metabuf, rootblkno, rootlevel,
					 wstate->inskey->allequalimage);
	_xt_blwritepage(wstate, metabuf, BTREE_METAPAGE);
}
 ...
 */
static void
_xt_load(BTWriteState *wstate, BTSpool *btspool, BTSpool *btspool2)
{
	...
	if (merge)
	{
		...
		for (;;)
		{
			...
			if (state == NULL)
				state = _xt_pagestate(wstate, 0);

			if (load1)
			{
				_xt_buildadd(wstate, state, itup, 0);
				itup = tuplesort_getindextuple(btspool->sortstate, true);
			}
			else
			{
				_xt_buildadd(wstate, state, itup2, 0);
				itup2 = tuplesort_getindextuple(btspool2->sortstate, true);
			}
	...
	else if (deduplicate)
	{
											   ...
											   true)) != NULL)
		{
			...
			if (state == NULL)
			{
				...
				_xt_dedup_start_pending(dstate, CopyIndexTuple(itup),
										InvalidOffsetNumber);
			}
			else if (_xt_keep_natts_fast(wstate->index, dstate->base,
										 itup) > keysz &&
					 _xt_dedup_save_htid(dstate, itup))
			{
				/*
			...
			else
			{
				 ...
				 * _xt_dedup_save_htid() opted to not merge current item into
				 * pending posting list.
				 */
				_xt_sort_dedup_finish_pending(wstate, state, dstate);
				pfree(dstate->base);
	...
	_xt_uppershutdown(wstate, state);
 ...
 * mode by passing it to _xt_end_parallel() at the very end of its index
 * build.  If not even a single worker process can be launched, this is
 * never set, and caller should proceed with a serial index build.
 */
static void
_xt_begin_parallel(BTBuildState *buildstate, bool isconcurrent, int request)
{
	ParallelContext *pcxt;
	int			scantuplesortstates;
	Snapshot	snapshot;
	...
	EnterParallelMode();
	Assert(request > 0);
	pcxt = CreateParallelContext("postgres", "_xt_parallel_build_main",
								 request);
	 ...
	 */
	estbtshared = _xt_parallel_estimate_shared(btspool->heap, snapshot);
	...
	if (leaderparticipates)
		_xt_leader_participate_as_worker(buildstate);
 ...
 */
static void
_xt_end_parallel(BTLeader *btleader)
{
	int			i;
 ...
 */
static Size
_xt_parallel_estimate_shared(Relation heap, Snapshot snapshot)
{
 ...
 *
 * When called, parallel heap scan started by _xt_begin_parallel() will
 ...
 */
static double
_xt_parallel_heapscan(BTBuildState *buildstate, bool *brokenhotchain)
{
 ...
 */
static void
_xt_leader_participate_as_worker(BTBuildState *buildstate)
{
	...
	_xt_parallel_scan_and_sort(leaderworker, leaderworker2, btleader->btshared,
 ...
 */
void
_xt_parallel_build_main(dsm_segment *seg, shm_toc *toc)
{
	char	   *sharedquery;
	BTSpool    *btspool;
	BTSpool    *btspool2;
	...
	_xt_parallel_scan_and_sort(btspool, btspool2, btshared, sharedsort,
 ...
 */
static void
_xt_parallel_scan_and_sort(BTSpool *btspool, BTSpool *btspool2,
						   BTShared *btshared, Sharedsort *sharedsort,
						   Sharedsort *sharedsort2, int sortmem, bool progress)
{
	...
	/* Fill in buildstate for _xt_build_callback() */
	...
	reltuples = table_index_build_scan(btspool->heap, btspool->index, indexInfo,
									   true, progress, _xt_build_callback,

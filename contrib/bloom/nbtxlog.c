...
/*
 * _xt_restore_page -- re-enter all the index tuples on a page
 *
 ...
 */
static void
_xt_restore_page(Page page, char *from, int len)
{
	...
	for (i = nitems - 1; i >= 0; i--)
	{
		if (PageAddItem(page, items[i], itemsizes[i], nitems - i,
			...
			elog(PANIC, "_xt_restore_page: cannot add item to page");
	}
}

static void
_xt_restore_meta(XLogReaderState *record, uint8 block_id)
{
	...
	_xt_pageinit(metapg, BufferGetPageSize(metabuf));
...
/*
 * _xt_clear_incomplete_split -- clear INCOMPLETE_SPLIT flag on a page
 *
 * This is a common subroutine of the redo functions of all the WAL record
 * types that can insert a downlink: insert, split, and newroot.
 */
static void
_xt_clear_incomplete_split(XLogReaderState *record, uint8 block_id)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	Buffer		buf;
				  ...
				  XLogReaderState *record)
{
	 ...
	 */
	if (!isleaf)
		_xt_clear_incomplete_split(record, 1);
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		else
		{
			 ...
			 *
			 * Use _xt_swap_posting() to repeat posting list split steps from
			...
			/* Use mutable, aligned newitem copy in _xt_swap_posting() */
			Assert(isleaf && postingoff > 0);
			newitem = CopyIndexTuple((IndexTuple) datapos);
			nposting = _xt_swap_posting(newitem, oposting, postingoff);
...
btree_xlog_split(bool newitemonleft, XLogReaderState *record)
{
	 ...
	 */
	if (!isleaf)
		_xt_clear_incomplete_split(record, 3);
	...
	_xt_pageinit(rpage, BufferGetPageSize(rbuf));
	...
	_xt_restore_page(rpage, datapos, datalen);
	...
	if (XLogReadBufferForRedo(record, 0, &buf) == BLK_NEEDS_REDO)
	{
		 ...
		 * items to that in item number order.  This mirrors how _xt_split()
		 * works.  Retaining the same physical order makes WAL consistency
		 * checking possible.  See also _xt_restore_page(), which does the
		...
		if (newitemonleft || xlrec->postingoff != 0)
		{
			...
			if (xlrec->postingoff != 0)
			{
				...
				nposting = _xt_swap_posting(newitem, oposting,
											xlrec->postingoff);
			}
		}
...
btree_xlog_dedup(XLogReaderState *record)
{
	...
	if (XLogReadBufferForRedo(record, 0, &buf) == BLK_NEEDS_REDO)
	{
			 ...
			 offnum = OffsetNumberNext(offnum))
		{
			...
			{
				if (!_xt_dedup_save_htid(state, itup))
					elog(ERROR, "deduplication failed to add heap tid to pending posting list");
			}
			else
			{
				_xt_dedup_finish_pending(newpage, state);
				_xt_dedup_start_pending(state, itup, offnum);
			}
		}
...
btree_xlog_mark_page_halfdead(uint8 info, XLogReaderState *record)
{
	...
	_xt_pageinit(page, BufferGetPageSize(buffer));
	pageop = BTPageGetOpaque(page);
...
btree_xlog_unlink_page(uint8 info, XLogReaderState *record)
{
	...
	_xt_pageinit(page, BufferGetPageSize(target));
	pageop = BTPageGetOpaque(page);
	...
	if (XLogRecHasBlockRef(record, 3))
	{
		...
		_xt_pageinit(page, BufferGetPageSize(leafbuf));
		pageop = BTPageGetOpaque(page);
 ...
 * _xt_getbuf() verifies it's safe using BTPageIsRecyclable(), but that's just
 ...
 * and reused for an entirely unrelated page inside _xt_split().  These
...
btree_mask(char *pagedata, BlockNumber blkno)
{
	...
	if (P_ISLEAF(maskopaq))
	{
		 ...
		 * _xt_killitems(), _xt_check_unique() for details.
		 */
		mask_lp_flags(page);
	}
	 ...
	 * _xt_delete_or_dedup_one_page(), _xt_killitems(), and _xt_check_unique()
	 * for details.
	 */

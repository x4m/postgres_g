...
static BTMetaPageData *_xt_getmeta(Relation rel, Buffer metabuf);
static void _xt_delitems_delete(Relation rel, Buffer buf,
								TransactionId snapshotConflictHorizon,
								bool isCatalogRel,
								OffsetNumber *deletable, int ndeletable,
								BTVacuumPosting *updatable, int nupdatable);
static char *_xt_delitems_update(BTVacuumPosting *updatable, int nupdatable,
								 OffsetNumber *updatedoffsets,
								 Size *updatedbuflen, bool needswal);
static bool _xt_mark_page_halfdead(Relation rel, Relation heaprel,
								   Buffer leafbuf, BTStack stack);
static bool _xt_unlink_halfdead_page(Relation rel, Buffer leafbuf,
									 BlockNumber scanblkno,
									 bool *rightsib_empty,
									 BTVacState *vstate);
static bool _xt_lock_subtree_parent(Relation rel, Relation heaprel,
									BlockNumber child, BTStack stack,
									Buffer *subtreeparent, OffsetNumber *poffset,
									BlockNumber *topparent,
									BlockNumber *topparentrightsib);
static void _xt_pendingfsm_add(BTVacState *vstate, BlockNumber target,
							   FullTransactionId safexid);

/*
 *	_xt_initmetapage() -- Fill a page buffer with a correct metapage image
 */
void
_xt_initmetapage(Page page, BlockNumber rootbknum, uint32 level,
				 bool allequalimage)
{
	BTMetaPageData *metad;
	BTPageOpaque metaopaque;

	_xt_pageinit(page, BLCKSZ);
...
/*
 *	_xt_upgrademetapage() -- Upgrade a meta-page from an old format to version
...
_xt_upgrademetapage(Page page)
{
	...
	/* Adjust pd_lower (see _xt_initmetapage() for details) */
	((PageHeader) page)->pd_lower =
		((char *) metad + sizeof(BTMetaPageData)) - (char *) page;
}
 ...
 * on-the-fly upgrade using _xt_upgrademetapage() can change the version field
 * and BTREE_NOVAC_VERSION specific fields without invalidating local cache.
 */
static BTMetaPageData *
_xt_getmeta(Relation rel, Buffer metabuf)
{
	Page		metapg;
	BTPageOpaque metaopaque;
	BTMetaPageData *metad;
...
_xt_vacuum_needs_cleanup(Relation rel)
{
	 ...
	 */
	metabuf = _xt_getbuf(rel, BTREE_METAPAGE, BT_READ);
	metapg = BufferGetPage(metabuf);
	metad = BTPageGetMeta(metapg);
...
/*
 * _xt_set_cleanup_info() -- Update metapage for btvacuumcleanup.
 *
 * Called at the end of btvacuumcleanup, when num_delpages value has been
 * finalized.
 */
void
_xt_set_cleanup_info(Relation rel, BlockNumber num_delpages)
{
	Buffer		metabuf;
	Page		metapg;
	BTMetaPageData *metad;
	 ...
	 * field.  _xt_vacuum_needs_cleanup() may even believe that this value
	...
	_xt_unlockbuf(rel, metabuf);
	_xt_lockbuf(rel, metabuf, BT_WRITE);

	START_CRIT_SECTION();

	/* upgrade meta-page if needed */
	if (metad->btm_version < BTREE_NOVAC_VERSION)
		_xt_upgrademetapage(metapg);
...
/*
 *	_xt_getroot() -- Get the root page of the btree.
 *
 ...
 *		pass NULL.  See _xt_allocbuf for an explanation.
 *
 ...
 */
Buffer
_xt_getroot(Relation rel, Relation heaprel, int access)
{
	Buffer		metabuf;
	Buffer		rootbuf;
	Page		rootpage;
	...
	if (rel->rd_amcache != NULL)
	{
		...
		rootbuf = _xt_getbuf(rel, rootblkno, BT_READ);
	...
	if (metad->btm_root == P_NONE)
	{
		...
		if (metad->btm_root != P_NONE)
		{
			 ...
			 */
			_xt_relbuf(rel, metabuf);
			return _xt_getroot(rel, heaprel, access);
		}
		 ...
		 */
		rootbuf = _xt_allocbuf(rel, heaprel);
		 ...
		 */
		_xt_unlockbuf(rel, rootbuf);
		_xt_lockbuf(rel, rootbuf, BT_READ);
	...
	else
	{
		 ...
		 * _xt_relandgetbuf call
		 */
		rootbuf = metabuf;

		for (;;)
		{
			rootbuf = _xt_relandgetbuf(rel, rootbuf, rootblkno, BT_READ);
			rootpage = BufferGetPage(rootbuf);
			rootopaque = BTPageGetOpaque(rootpage);
...
/*
 *	_xt_gettrueroot() -- Get the true root page of the btree.
 *
 *		This is the same as the BT_READ case of _xt_getroot(), except
...
/*
 *	_xt_getrootheight() -- Get the height of the btree search tree.
 *
 ...
 */
int
_xt_getrootheight(Relation rel)
{
	BTMetaPageData *metad;
	...
	if (rel->rd_amcache == NULL)
	{
		...
		/*
		 * If there's no root page yet, _xt_getroot() doesn't expect a cache
		 * to be made, so just stop here and report the index height is zero.
		 * (XXX perhaps _xt_getroot() should be changed to allow this case.)
		 */
		if (metad->btm_root == P_NONE)
		{
			_xt_relbuf(rel, metabuf);
...
/*
 *	_xt_metaversion() -- Get version/status info from metapage.
 *
 ...
 */
void
_xt_metaversion(Relation rel, bool *heapkeyspace, bool *allequalimage)
{
	BTMetaPageData *metad;
	...
	if (rel->rd_amcache == NULL)
	{
		...
		/*
		 * If there's no root page yet, _xt_getroot() doesn't expect a cache
		 * to be made, so just stop here.  (XXX perhaps _xt_getroot() should
		 ...
		 * Cache the metapage data for next time
		 *
		 * An on-the-fly version upgrade performed by _xt_upgrademetapage()
...
/*
 *	_xt_checkpage() -- Verify that a freshly-read page looks sane.
 */
void
_xt_checkpage(Relation rel, Buffer buf)
{
...
/*
 *	_xt_getbuf() -- Get an existing block in a buffer, for read or write.
 *
 ...
 *		_xt_checkpage to sanity-check the page, and perform Valgrind
 ...
 */
Buffer
_xt_getbuf(Relation rel, BlockNumber blkno, int access)
{
	Buffer		buf;
	...
	_xt_lockbuf(rel, buf, access);
	_xt_checkpage(rel, buf);

	return buf;
}

/*
 *	_xt_allocbuf() -- Allocate a new block/page.
 *
 ...
 */
Buffer
_xt_allocbuf(Relation rel, Relation heaprel)
{
	Buffer		buf;
	BlockNumber blkno;
	Page		page;
	 ...
	 * it's entirely likely that _xt_split will call us while holding a lock
	...
	for (;;)
	{
		...
		if (blkno == InvalidBlockNumber)
			break;
		buf = ReadBuffer(rel, blkno);
		if (_xt_conditionallockbuf(rel, buf))
		{
			...
			if (PageIsNew(page))
			{
				...
				_xt_pageinit(page, BufferGetPageSize(buf));
				return buf;
			}
	 ...
	 * This forces us to repeat the valgrind request that _xt_lockbuf()
	 * otherwise would make, as we can't use _xt_lockbuf() without introducing
	 * a race.
	 */
...
/*
 *	_xt_relandgetbuf() -- release a locked buffer and get another one.
 *
 * This is equivalent to _xt_relbuf followed by _xt_getbuf.  Also, if obuf is
 * InvalidBuffer then it reduces to just _xt_getbuf; allowing this case
 * simplifies some callers.
 *
 * The original motivation for using this was to avoid two entries to the
 * bufmgr when one would do.  However, now it's mainly just a notational
 * convenience.  The only case where it saves work over _xt_relbuf/_xt_getbuf
 * is when the target page is the same one already in the buffer.
 */
Buffer
_xt_relandgetbuf(Relation rel, Buffer obuf, BlockNumber blkno, int access)
{
	Buffer		buf;

	Assert(BlockNumberIsValid(blkno));
	if (BufferIsValid(obuf))
		_xt_unlockbuf(rel, obuf);
...
/*
 *	_xt_relbuf() -- release a locked buffer.
 *
 * Lock and pin (refcount) are both dropped.
 */
void
_xt_relbuf(Relation rel, Buffer buf)
{
	_xt_unlockbuf(rel, buf);
	ReleaseBuffer(buf);
}

/*
 *	_xt_lockbuf() -- lock a pinned buffer.
 *
 ...
 *
 * Note: Caller may need to call _xt_checkpage() with buf when pin on buf
 * wasn't originally acquired in _xt_getbuf() or _xt_relandgetbuf().
 */
void
_xt_lockbuf(Relation rel, Buffer buf, int access)
{
	/* LockBuffer() asserts that pin is held by this backend */
	LockBuffer(buf, access);

	/*
	 * It doesn't matter that _xt_unlockbuf() won't get called in the event of
...
/*
 *	_xt_unlockbuf() -- unlock a pinned buffer.
 */
void
_xt_unlockbuf(Relation rel, Buffer buf)
{
	/*
...
/*
 *	_xt_conditionallockbuf() -- conditionally BT_WRITE lock pinned
 *	buffer.
 *
 * Note: Caller may need to call _xt_checkpage() with buf when pin on buf
 * wasn't originally acquired in _xt_getbuf() or _xt_relandgetbuf().
 */
bool
_xt_conditionallockbuf(Relation rel, Buffer buf)
{
...
/*
 *	_xt_upgradelockbufcleanup() -- upgrade lock to a full cleanup lock.
 */
void
_xt_upgradelockbufcleanup(Relation rel, Buffer buf)
{
	/*
	 * Buffer is pinned and locked, which means that it is expected to be
	 * defined and addressable.  Check that proactively.
...
/*
 *	_xt_pageinit() -- Initialize a new page.
 *
 * On return, the page header is initialized; data space is empty;
 * special space is zeroed out.
 */
void
_xt_pageinit(Page page, Size size)
{
 ...
 */
void
_xt_delitems_vacuum(Relation rel, Buffer buf,
					OffsetNumber *deletable, int ndeletable,
					BTVacuumPosting *updatable, int nupdatable)
{
	Page		page = BufferGetPage(buf);
	...
	if (nupdatable > 0)
		updatedbuf = _xt_delitems_update(updatable, nupdatable,
	...
	if (updatedbuf != NULL)
		pfree(updatedbuf);
	/* free tuples allocated within _xt_delitems_update() */
	for (int i = 0; i < nupdatable; i++)
 ...
 * (a version that lacks the TIDs that are to be deleted).
 *
 * This is nearly the same as _xt_delitems_vacuum as far as what it does to
 ...
 * conflicts.  The other difference is that only _xt_delitems_vacuum will
 * clear page's VACUUM cycle ID.
 */
static void
_xt_delitems_delete(Relation rel, Buffer buf,
					...
					BTVacuumPosting *updatable, int nupdatable)
{
	...
	/* Handle updates and deletes just like _xt_delitems_vacuum */
	for (int i = 0; i < nupdatable; i++)
	{
		OffsetNumber updatedoffset = updatedoffsets[i];
		IndexTuple	itup;
	...
	/*
	 * Unlike _xt_delitems_vacuum, we *must not* clear the vacuum cycle ID at
 ...
 * the tuple.  Performs steps common to both _xt_delitems_vacuum and
 * _xt_delitems_delete.  These steps must take place before each function's
 * critical section begins.
 *
 * updatable and nupdatable are inputs, though note that we will use
 * _xt_update_posting() to replace the original itup with a pointer to a final
 ...
 */
static char *
_xt_delitems_update(BTVacuumPosting *updatable, int nupdatable,
...
/*
 * Comparator used by _xt_delitems_delete_check() to restore deltids array
 * back to its original leaf-page-wise sort order
 */
static int
_xt_delitems_cmp(const void *a, const void *b)
{
 ...
 */
void
_xt_delitems_delete_check(Relation rel, Buffer buf, Relation heapRel,
						  TM_IndexDeleteOp *delstate)
{
	...
	/*
	 * Construct a leaf-page-wise description of what _xt_delitems_delete()
	...
	_xt_delitems_delete(rel, buf, snapshotConflictHorizon, isCatalogRel,
 ...
 */
static bool
_xt_leftsib_splitflag(Relation rel, BlockNumber leftsib, BlockNumber target)
{
	Buffer		buf;
	Page		page;
	BTPageOpaque opaque;
	...
	buf = _xt_getbuf(rel, leftsib, BT_READ);
	page = BufferGetPage(buf);
	opaque = BTPageGetOpaque(page);
 ...
 * again).
 *
 * _xt_leftsib_splitflag() is called for both leaf pages and internal pages.
 * _xt_rightsib_halfdeadflag() is only called for leaf pages, though.  This is
 ...
 */
static bool
_xt_rightsib_halfdeadflag(Relation rel, BlockNumber leafrightsib)
{
	Buffer		buf;
	Page		page;
	BTPageOpaque opaque;
	...
	buf = _xt_getbuf(rel, leafrightsib, BT_READ);
	page = BufferGetPage(buf);
	opaque = BTPageGetOpaque(page);
...
/*
 * _xt_pagedel() -- Delete a leaf page from the b-tree, if legal to do so.
 *
 ...
 */
void
_xt_pagedel(Relation rel, Buffer leafbuf, BTVacState *vstate)
{
	...
	for (;;)
	{
		 ...
		 * _xt_leftsib_splitflag().  On the first iteration, we temporarily
		...
		if (!P_ISHALFDEAD(opaque))
		{
			...
			if (!stack)
			{
				...
				if (_xt_leftsib_splitflag(rel, leftsib, leafblkno))
				{
				 ...
				 * original lock was dropped, but before _xt_search finished
				 ...
				 */
				_xt_lockbuf(rel, leafbuf, BT_WRITE);
				continue;
			}
			 ...
			 * deleted.  If it's safe then _xt_mark_page_halfdead will also
			 * perform the first phase of deletion, which includes marking the
			 * leafbuf page half-dead.
			 */
			Assert(P_ISLEAF(opaque) && !P_IGNORE(opaque));
			if (!_xt_mark_page_halfdead(rel, vstate->info->heaprel, leafbuf,
										stack))
			{
				_xt_relbuf(rel, leafbuf);
				return;
		 ...
		 * _xt_unlink_halfdead_page unlinks the topmost page from the subtree,
		...
		{
			/* Check for interrupts in _xt_unlink_halfdead_page */
			if (!_xt_unlink_halfdead_page(rel, leafbuf, scanblkno,
										  &rightsib_empty, vstate))
			{
				/*
				 * _xt_unlink_halfdead_page should never fail, since we
				 * established that deletion is generally safe in
				 * _xt_mark_page_halfdead -- index must be corrupt.
				 *
				 * Note that _xt_unlink_halfdead_page already released the
		 ...
		 *
		 * Note: This relies on the assumption that _xt_getstackbuf() will be
		 ...
		 * original leaf page reached by _xt_search()). It will even update
		 * the descent stack each time we loop around, avoiding repeated work.
		 */
		if (!rightsib_empty)
			break;

		leafbuf = _xt_getbuf(rel, rightsib, BT_WRITE);
	}
}
 ...
 * call to _xt_lock_subtree_parent will need to allocate a new index page to
 * complete a page split.  Every call to _xt_allocbuf needs to pass a heaprel.
 *
 ...
 */
static bool
_xt_mark_page_halfdead(Relation rel, Relation heaprel, Buffer leafbuf,
					   BTStack stack)
{
	 ...
	 */
	if (_xt_rightsib_halfdeadflag(rel, leafrightsib))
	{
	 ...
	 * "subtree parent").  _xt_lock_subtree_parent() locks the subtree parent
	...
	if (!_xt_lock_subtree_parent(rel, heaprel, leafblkno, stack,
	...
	/*
	 * This is just an assertion because _xt_lock_subtree_parent should have
	 ...
	 * where _xt_lock_subtree_parent() cannot "re-find" leafbuf's downlink.)
	 */
 ...
 */
static bool
_xt_unlink_halfdead_page(Relation rel, Buffer leafbuf, BlockNumber scanblkno,
						 bool *rightsib_empty, BTVacState *vstate)
{
	...
	else
	{
		...
		buf = _xt_getbuf(rel, target, BT_READ);
		page = BufferGetPage(buf);
		opaque = BTPageGetOpaque(page);
		leftsib = opaque->btpo_prev;
	...
	if (leftsib != P_NONE)
	{
		lbuf = _xt_getbuf(rel, leftsib, BT_WRITE);
	 ...
	 */
	rightsib = opaque->btpo_next;
	rbuf = _xt_getbuf(rel, rightsib, BT_WRITE);
	...
	*rightsib_empty = (P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page));
	...
	if (leftsib == P_NONE && rightsib_is_rightmost)
	{
		...
		if (P_RIGHTMOST(opaque))
		{
			...
			metabuf = _xt_getbuf(rel, BTREE_METAPAGE, BT_WRITE);
			metapg = BufferGetPage(metabuf);
			metad = BTPageGetMeta(metapg);
	 ...
	 * call.
	 */
	_xt_pendingfsm_add(vstate, target, safexid);
 ...
 */
static bool
_xt_lock_subtree_parent(Relation rel, Relation heaprel, BlockNumber child,
						...
						BlockNumber *topparentrightsib)
{
	 ...
	 */
	pbuf = _xt_getstackbuf(rel, heaprel, stack, child);
	if (pbuf == InvalidBuffer)
	{
		 ...
		 *
		 * Note: _xt_getstackbuf() recovers from concurrent page splits that
	...
	/*
	 * _xt_getstackbuf() completes page splits on returned parent buffer when
	 * required.
	 *
	...
	*topparentrightsib = opaque->btpo_next;
	 ...
	 */
	if (_xt_leftsib_splitflag(rel, leftsibparent, parent))
		return false;

	/* Recurse to examine child page's grandparent page */
	return _xt_lock_subtree_parent(rel, heaprel, parent, stack->bts_parent,
								   subtreeparent, poffset,
								   topparent, topparentrightsib);
}

/*
 * Initialize local memory state used by VACUUM for _xt_pendingfsm_finalize
 * optimization.
 *
 ...
 */
void
_xt_pendingfsm_init(Relation rel, BTVacState *vstate, bool cleanuponly)
{
	int64		maxbufsize;
...
/*
 * Place any newly deleted pages (i.e. pages that _xt_pagedel() deleted during
 ...
 * takes place.
 *
 * Frees memory allocated by _xt_pendingfsm_init(), if any.
 */
void
_xt_pendingfsm_finalize(Relation rel, BTVacState *vstate)
{
	IndexBulkDeleteResult *stats = vstate->stats;
	...
	for (int i = 0; i < vstate->npendingpages; i++)
	{
		 ...
		 * must be non-recyclable too, since _xt_pendingfsm_add() adds pages
 ...
 * call, for use in _xt_pendingfsm_finalize()
 */
static void
_xt_pendingfsm_add(BTVacState *vstate,
				   BlockNumber target,
				   FullTransactionId safexid)
{
	...
	/*
	 * Verify an assumption made by _xt_pendingfsm_finalize(): pages from the
	 ...
	 * optimization in _xt_pendingfsm_init().
	 */

...
static BTStack _xt_search_insert(Relation rel, Relation heaprel,
								 BTInsertState insertstate);
static TransactionId _xt_check_unique(Relation rel, BTInsertState insertstate,
									  Relation heapRel,
									  IndexUniqueCheck checkUnique, bool *is_unique,
									  uint32 *speculativeToken);
static OffsetNumber _xt_findinsertloc(Relation rel,
									  BTInsertState insertstate,
									  bool checkingunique,
									  bool indexUnchanged,
									  BTStack stack,
									  Relation heapRel);
static void _xt_stepright(Relation rel, Relation heaprel,
						  BTInsertState insertstate, BTStack stack);
static void _xt_insertonpg(Relation rel, Relation heaprel, BTScanInsert itup_key,
						   Buffer buf,
						   Buffer cbuf,
						   BTStack stack,
						   IndexTuple itup,
...
static Buffer _xt_split(Relation rel, Relation heaprel, BTScanInsert itup_key,
						Buffer buf, Buffer cbuf, OffsetNumber newitemoff,
						Size newitemsz, IndexTuple newitem, IndexTuple orignewitem,
						IndexTuple nposting, uint16 postingoff);
static void _xt_insert_parent(Relation rel, Relation heaprel, Buffer buf,
							  Buffer rbuf, BTStack stack, bool isroot, bool isonly);
static Buffer _xt_newlevel(Relation rel, Relation heaprel, Buffer lbuf, Buffer rbuf);
static inline bool _xt_pgaddtup(Page page, Size itemsize, IndexTuple itup,
								OffsetNumber itup_off, bool newfirstdataitem);
static void _xt_delete_or_dedup_one_page(Relation rel, Relation heapRel,
										 BTInsertState insertstate,
										 bool simpleonly, bool checkingunique,
										 bool uniquedup, bool indexUnchanged);
static void _xt_simpledel_pass(Relation rel, Buffer buffer, Relation heapRel,
							   OffsetNumber *deletable, int ndeletable,
							   IndexTuple newitem, OffsetNumber minoff,
							   OffsetNumber maxoff);
static BlockNumber *_xt_deadblocks(Page page, OffsetNumber *deletable,
								   int ndeletable, IndexTuple newitem,
								   int *nblocks);
static inline int _xt_blk_cmp(const void *arg1, const void *arg2);

/*
 *	_xt_doinsert() -- Handle insertion of a single index tuple in the tree.
 *
 ...
 */
bool
_xt_doinsert(Relation rel, IndexTuple itup,
			 ...
			 Relation heapRel)
{
	...
	itup_key = _xt_mkscankey(rel, itup);
	...
	if (checkingunique)
	{
		...
		else
		{
			 ...
			 * _xt_findinsertloc() heapkeyspace path when a unique index has a
...
search:
	 ...
	 */
	stack = _xt_search_insert(rel, heapRel, &insertstate);
	 ...
	 * needed) here.
	 *
	 * It might be necessary to check a page to the right in _xt_check_unique,
	 ...
	 * of _xt_findsplitloc with duplicate tuples -- a group of duplicates can
	 * only be allowed to cross a page boundary when there is no candidate
	 * leaf page split point that avoids it.  Also, _xt_check_unique can use
	 ...
	 * page).
	 *
	 * NOTE: obviously, _xt_check_unique can only detect keys that are already
	 ...
	 * insertion.  (This requires some care in _xt_findinsertloc.)
	 *
	...
	if (checkingunique)
	{
		...
		xwait = _xt_check_unique(rel, &insertstate, heapRel, checkUnique,
								 &is_unique, &speculativeToken);

		if (unlikely(TransactionIdIsValid(xwait)))
		{
			/* Have to wait for the other guy ... */
			_xt_relbuf(rel, insertstate.buf);
			insertstate.buf = InvalidBuffer;
	...
	if (checkUnique != UNIQUE_CHECK_EXISTING)
	{
		 ...
		 * search bounds established within _xt_check_unique when insertion is
		 * checkingunique.
		 */
		newitemoff = _xt_findinsertloc(rel, &insertstate, checkingunique,
									   indexUnchanged, stack, heapRel);
		_xt_insertonpg(rel, heapRel, itup_key, insertstate.buf, InvalidBuffer,
...
/*
 *	_xt_search_insert() -- _xt_search() wrapper for inserts
 *
 ...
 * page is maintained within _xt_insertonpg(), and used here.  The cache is
 ...
 */
static BTStack
_xt_search_insert(Relation rel, Relation heaprel, BTInsertState insertstate)
{
	...
	{
		/* Simulate a _xt_getbuf() call with conditional locking */
		insertstate->buf = ReadBuffer(rel, RelationGetTargetBlock(rel));
		if (_xt_conditionallockbuf(rel, insertstate->buf))
		{
			Page		page;
			BTPageOpaque opaque;

			_xt_checkpage(rel, insertstate->buf);
			...
			if (P_RIGHTMOST(opaque) &&
				...
				_xt_compare(rel, insertstate->itup_key, page, P_HIKEY) > 0)
			{
				/*
				 ...
				 *
				 * Note that _xt_insert_parent() has an assertion that catches
	...
	return _xt_search(rel, heaprel, insertstate->itup_key, &insertstate->buf,
					  BT_WRITE);
}

/*
 *	_xt_check_unique() -- Check for violation of unique index constraint
 *
 ...
 * _xt_findinsertloc() to reuse most of the binary search work we do
 * here.
 *
...
_xt_check_unique(Relation rel, BTInsertState insertstate, Relation heapRel,
				 IndexUniqueCheck checkUnique, bool *is_unique,
				 uint32 *speculativeToken)
{
	...
	*is_unique = true;
	 ...
	 * in the fastpath below, but also in the _xt_findinsertloc() call later.
	 */
	Assert(!insertstate->bounds_valid);
	offset = _xt_binsrch_insert(rel, insertstate);
	...
	for (;;)
	{
		...
		if (offset <= maxoff)
		{
			 ...
			 * _xt_compare() directly when there are no duplicates, as long as
			...
			if (nbuf == InvalidBuffer && offset == insertstate->stricthigh)
			{
				...
				Assert(_xt_compare(rel, itup_key, page, offset) < 0);
				break;
			}
			 ...
			 * many killed items with the same key; running _xt_compare() on
			 * each killed item gets expensive.  Just advance over killed
			 * items as quickly as we can.  We only apply _xt_compare() when
			 * we get to a non-killed item.  We could reuse the bounds to
			 * avoid _xt_compare() calls for known equal tuples, but it
			...
			if (inposting || !ItemIdIsDead(curitemid))
			{
				...
				if (!inposting)
				{
					...
					if (_xt_compare(rel, itup_key, page, offset) != 0)
		...
		else
		{
			...
			if (P_RIGHTMOST(opaque))
				break;
			highkeycmp = _xt_compare(rel, itup_key, page, P_HIKEY);
			Assert(highkeycmp <= 0);
			if (highkeycmp != 0)
				break;
			...
			for (;;)
			{
				...
				nbuf = _xt_relandgetbuf(rel, nbuf, nblkno, BT_READ);
...
/*
 *	_xt_findinsertloc() -- Finds an insert location for a tuple
 *
 ...
 *		the offset within that page is returned.  If _xt_findinsertloc needed
 ...
 *		_xt_check_unique() already.
 */
static OffsetNumber
_xt_findinsertloc(Relation rel,
				  BTInsertState insertstate,
				  bool checkingunique,
				  bool indexUnchanged,
				  BTStack stack,
				  Relation heapRel)
{
	...
	if (unlikely(insertstate->itemsz > BTMaxItemSize(page)))
		_xt_check_third_page(rel, heapRel, itup_key->heapkeyspace, page,
							 insertstate->itup);
	...
	if (itup_key->heapkeyspace)
	{
		 ...
		 * used when we called _xt_search().  scantid can only be set after
		 * _xt_check_unique() has checked for duplicates.  The buffer
		...
		if (checkingunique)
		{
			...
			{
				/* Encountered a duplicate in _xt_check_unique() */
				Assert(insertstate->bounds_valid);
				uniquedup = true;
			}
			...
			for (;;)
			{
				 ...
				 *
				 * The earlier _xt_check_unique() call may well have
				...
				if (P_RIGHTMOST(opaque) ||
					_xt_compare(rel, itup_key, page, P_HIKEY) <= 0)
					break;

				_xt_stepright(rel, heapRel, insertstate, stack);
		...
		if (PageGetFreeSpace(page) < insertstate->itemsz)
			_xt_delete_or_dedup_one_page(rel, heapRel, insertstate, false,
	...
	else
	{
		...
		while (PageGetFreeSpace(page) < insertstate->itemsz)
		{
			...
			if (P_HAS_GARBAGE(opaque))
			{
				...
				_xt_delete_or_dedup_one_page(rel, heapRel, insertstate, true,
											 false, false, false);
			 ...
			 *
			 * The earlier _xt_check_unique() call may well have established a
	...
	newitemoff = _xt_binsrch_insert(rel, insertstate);
 ...
 * else someone else's _xt_check_unique scan could fail to see our insertion.
 ...
 */
static void
_xt_stepright(Relation rel, Relation heaprel, BTInsertState insertstate,
			  BTStack stack)
{
	Page		page;
	BTPageOpaque opaque;
	...
	for (;;)
	{
		rbuf = _xt_relandgetbuf(rel, rbuf, rblkno, BT_WRITE);
		...
		{
			_xt_finish_split(rel, heaprel, rbuf, stack);
			rbuf = InvalidBuffer;
			continue;
		}
...
/*----------
 *	_xt_insertonpg() -- Insert a tuple on a particular page in the index.
 *
			   ...
			   bool split_only_page)
{
	 ...
	 * all times.  Only _xt_split() and _xt_newlevel() should add items that
	...
	if (postingoff != 0)
	{
		...
		/*
		 * postingoff value comes from earlier call to _xt_binsrch_posting().
		...
		nposting = _xt_swap_posting(itup, oposting, postingoff);
	...
	if (PageGetFreeSpace(page) < itemsz)
	{
		...
		rbuf = _xt_split(rel, heaprel, itup_key, buf, cbuf, newitemoff, itemsz,
		 ...
		 *----------
		 */
		_xt_insert_parent(rel, heaprel, buf, rbuf, stack, isroot, isonly);
	}
	else
	{
		 ...
		 * metapage here --- see comments for _xt_newlevel().
		 */
		 ...
		 * may be used by a future inserter within _xt_search_insert().
		 */
		 ...
		 * call _xt_getrootheight while holding a buffer lock.
		 */
		if (BlockNumberIsValid(blockcache) &&
			_xt_getrootheight(rel) >= BTREE_FASTPATH_MIN_LEVEL)
...
/*
 *	_xt_split() -- split a page in the btree.
 *
 ...
 */
static Buffer
_xt_split(Relation rel, Relation heaprel, BTScanInsert itup_key, Buffer buf,
		  ...
		  IndexTuple orignewitem, IndexTuple nposting, uint16 postingoff)
{
	 ...
	 * case.
	 */
	firstrightoff = _xt_findsplitloc(rel, origpage, newitemoff, newitemsz,
									 newitem, &newitemonleft);

	/* Allocate temp buffer for leftpage */
	leftpage = PageGetTempPage(origpage);
	_xt_pageinit(leftpage, BufferGetPageSize(buf));
	 ...
	 *
	 * Note: _xt_findsplitloc() should have compensated for coinciding posting
	...
	if (isleaf)
	{
		...
		lefthighkey = _xt_truncate(rel, lastleft, firstright, itup_key);
	...
	else
	{
		 ...
		 * below, though.  This happens during the call to _xt_pgaddtup() for
	 ...
	 */
	rbuf = _xt_allocbuf(rel, heaprel);
	rightpage = BufferGetPage(rbuf);
	rightpagenumber = BufferGetBlockNumber(rbuf);
	/* rightpage was initialized by _xt_getbuf */
	...
	lopaque->btpo_cycleid = _xt_vacuum_cycleid(rel);
	 ...
	 * order, for the benefit of _xt_restore_page().
	 */
	for (i = P_FIRSTDATAKEY(oopaque); i <= maxoff; i = OffsetNumberNext(i))
	{
		...
		else if (i == newitemoff)
		{
			if (newitemonleft)
			{
				...
				if (!_xt_pgaddtup(leftpage, newitemsz, newitem, afterleftoff,
								  false))
				{
					memset(rightpage, 0, BufferGetPageSize(rbuf));
			...
			{
				Assert(newitemoff >= firstrightoff);
				if (!_xt_pgaddtup(rightpage, newitemsz, newitem, afterrightoff,
		...
		if (i < firstrightoff)
		{
			if (!_xt_pgaddtup(leftpage, itemsz, dataitem, afterleftoff, false))
			{
				memset(rightpage, 0, BufferGetPageSize(rbuf));
		...
		{
			if (!_xt_pgaddtup(rightpage, itemsz, dataitem, afterrightoff,
							  afterrightoff == minusinfoff))
			{
				memset(rightpage, 0, BufferGetPageSize(rbuf));
	...
	if (!isrightmost)
	{
		sbuf = _xt_getbuf(rel, oopaque->btpo_next, BT_WRITE);
	...
	if (RelationNeedsWAL(rel))
	{
		 ...
		 * must reconstruct nposting and newitem using _xt_swap_posting().
		 *
		 ...
		 * _xt_restore_page().  The whole right page will be recreated.
		 *
...
/*
 * _xt_insert_parent() -- Insert downlink into parent, completing split.
 *
 ...
 */
static void
_xt_insert_parent(Relation rel,
				  Relation heaprel,
				  Buffer buf,
				  Buffer rbuf,
				  BTStack stack,
				  ...
				  bool isonly)
{
	...
	if (isroot)
	{
		...
		rootbuf = _xt_newlevel(rel, heaprel, buf, rbuf);
	...
	else
	{
		...
		if (stack == NULL)
		{
			 ...
			 * rejected within _xt_search_insert() when the rightmost leaf
			 * page will split, since it's faster to go through _xt_search()
			...
			pbuf = _xt_get_endpoint(rel, opaque->btpo_level + 1, false);
		 ...
		 * our initial _xt_search() descent.  _xt_getstackbuf() will detect
		 * and recover from this, updating the stack, which ensures that the
		 * new downlink will be inserted at the correct offset. Even buf's
		 * parent may have changed.
		 */
		pbuf = _xt_getstackbuf(rel, heaprel, stack, bknum);
		...
		_xt_insertonpg(rel, heaprel, NULL, pbuf, buf, stack->bts_parent,
...
/*
 * _xt_finish_split() -- Finish an incomplete split
 *
 * A crash or other failure can leave a split incomplete.  The insertion
 * routines won't allow to insert on a page that is incompletely split.
 * Before inserting on such a page, call _xt_finish_split().
 *
 ...
 */
void
_xt_finish_split(Relation rel, Relation heaprel, Buffer lbuf, BTStack stack)
{
	...
	rbuf = _xt_getbuf(rel, lpageop->btpo_next, BT_WRITE);
	...
	_xt_insert_parent(rel, heaprel, lbuf, rbuf, stack, wasroot, wasonly);
}

/*
 *	_xt_getstackbuf() -- Walk back up the tree one step, and find the pivot
 ...
 *		stack.  For example, the checkingunique _xt_doinsert() case may
 ...
 */
Buffer
_xt_getstackbuf(Relation rel, Relation heaprel, BTStack stack, BlockNumber child)
{
	BlockNumber blkno;
	OffsetNumber start;
	...
	for (;;)
	{
		...
		buf = _xt_getbuf(rel, blkno, BT_WRITE);
		page = BufferGetPage(buf);
		opaque = BTPageGetOpaque(page);
		...
		if (P_INCOMPLETE_SPLIT(opaque))
		{
			_xt_finish_split(rel, heaprel, buf, stack->bts_parent);
			continue;
		}
...
/*
 *	_xt_newlevel() -- Create a new level above root page.
 *
 ...
 */
static Buffer
_xt_newlevel(Relation rel, Relation heaprel, Buffer lbuf, Buffer rbuf)
{
	Buffer		rootbuf;
	Page		lpage,
				rootpage;
...
/*
 *	_xt_pgaddtup() -- add a data item to a particular page during split.
 *
 ...
 *		_xt_split()) has a key that must be treated as minus infinity after
...
/*
 * _xt_delete_or_dedup_one_page - Try to avoid a leaf page split.
 *
 ...
 */
static void
_xt_delete_or_dedup_one_page(Relation rel, Relation heapRel,
							 BTInsertState insertstate,
							 bool simpleonly, bool checkingunique,
							 bool uniquedup, bool indexUnchanged)
{
	...
	if (ndeletable > 0)
	{
		_xt_simpledel_pass(rel, buffer, heapRel, deletable, ndeletable,
	...
	if ((indexUnchanged || uniquedup) &&
		_xt_bottomupdel_pass(rel, buffer, heapRel, insertstate->itemsz))
		return;

	/* Perform deduplication pass (when enabled and index-is-allequalimage) */
	if (BTGetDeduplicateItems(rel) && itup_key->allequalimage)
		_xt_dedup_pass(rel, buffer, insertstate->itup, insertstate->itemsz,
					   (indexUnchanged || uniquedup));
}

/*
 * _xt_simpledel_pass - Simple index tuple deletion pass.
 *
 ...
 */
static void
_xt_simpledel_pass(Relation rel, Buffer buffer, Relation heapRel,
				   ...
				   OffsetNumber minoff, OffsetNumber maxoff)
{
	...
	deadblocks = _xt_deadblocks(page, deletable, ndeletable, newitem,
								&ndeadblocks);
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		if (!BTreeTupleIsPosting(itup))
		{
			...
			match = bsearch(&tidblock, deadblocks, ndeadblocks,
							sizeof(BlockNumber), _xt_blk_cmp);
	...
	_xt_delitems_delete_check(rel, buffer, heapRel, &delstate);
...
/*
 * _xt_deadblocks() -- Get LP_DEAD related table blocks.
 *
 ...
 */
static BlockNumber *
_xt_deadblocks(Page page, OffsetNumber *deletable, int ndeletable,
			   IndexTuple newitem, int *nblocks)
{
	...
	qsort(tidblocks, ntids, sizeof(BlockNumber), _xt_blk_cmp);
	*nblocks = qunique(tidblocks, ntids, sizeof(BlockNumber), _xt_blk_cmp);

	return tidblocks;
}

/*
 * _xt_blk_cmp() -- qsort comparison function for _xt_simpledel_pass
 */
static inline int
_xt_blk_cmp(const void *arg1, const void *arg2)
{

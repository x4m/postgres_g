 ...
 * scan to advance it via another call to _xt_first.
 *
...
static void _xtvacuumscan(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
						 IndexBulkDeleteCallback callback, void *callback_state,
						 BTCycleId cycleid);
static void _xtvacuumpage(BTVacState *vstate, BlockNumber scanblkno);
static BTVacuumPosting _xtreevacuumposting(BTVacState *vstate,
										  IndexTuple posting,
										  OffsetNumber updatedoffset,
										  int *nremaining);


PG_FUNCTION_INFO_V1(xthandler);
/*
 * Btree handler function: return IndexAmRoutine with access method parameters
 * and callbacks.
 */
Datum
xthandler(PG_FUNCTION_ARGS)
{
	...
	amroutine->ambuild = _xtbuild;
	amroutine->ambuildempty = _xtbuildempty;
	amroutine->aminsert = _xtinsert;
	amroutine->aminsertcleanup = NULL;
	amroutine->ambulkdelete = _xtbulkdelete;
	amroutine->amvacuumcleanup = _xtvacuumcleanup;
	amroutine->amcanreturn = _xtcanreturn;
	amroutine->amcostestimate = btcostestimate;
	amroutine->amoptions = _xtoptions;
	amroutine->amproperty = _xtproperty;
	amroutine->ambuildphasename = _xtbuildphasename;
	amroutine->amvalidate = _xtvalidate;
	amroutine->amadjustmembers = _xtadjustmembers;
	amroutine->ambeginscan = _xtbeginscan;
	amroutine->amrescan = _xtrescan;
	amroutine->amgettuple = _xtgettuple;
	amroutine->amgetbitmap = _xtgetbitmap;
	amroutine->amendscan = _xtendscan;
	amroutine->ammarkpos = _xtmarkpos;
	amroutine->amrestrpos = _xtrestrpos;
	amroutine->amestimateparallelscan = _xtestimateparallelscan;
	amroutine->aminitparallelscan = _xtinitparallelscan;
	amroutine->amparallelrescan = _xtparallelrescan;
 ...
 */
void
_xtbuildempty(Relation index)
{
	bool		allequalimage = _xt_allequalimage(index, false);
	...
	_xt_initmetapage((Page) metabuf, P_NONE, 0, allequalimage);
...
/*
 *	_xtinsert() -- insert an index tuple into a _xtree.
 *
 *		Descend the tree recursively, find the appropriate location for our
 *		new tuple, and put it there.
 */
bool
_xtinsert(Relation rel, Datum *values, bool *isnull,
		 ...
		 IndexInfo *indexInfo)
{
	...
	result = _xt_doinsert(rel, itup, checkUnique, indexUnchanged, heapRel);
...
/*
 *	_xtgettuple() -- Get the next tuple in the scan.
 */
bool
_xtgettuple(IndexScanDesc scan, ScanDirection dir)
{
	...
	do
	{
		 ...
		 * _xt_first() to get the first item in the scan.
		 */
		if (!BTScanPosIsValid(so->currPos))
			res = _xt_first(scan, dir);
		else
		{
			/*
	...
	} while (so->numArrayKeys && _xt_start_prim_scan(scan, dir));
 ...
 */
int64
_xtgetbitmap(IndexScanDesc scan, TIDBitmap *tbm)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	int64		ntids = 0;
	ItemPointer heapTid;
	...
	do
	{
		...
		if (_xt_first(scan, ForwardScanDirection))
		{
			...
			for (;;)
			{
				 ...
				 * easy case in _xt_next().
				 */
				if (++so->currPos.itemIndex > so->currPos.lastItem)
				{
					/* let _xt_next do the heavy lifting */
					if (!_xt_next(scan, ForwardScanDirection))
						break;
				}
	...
	} while (so->numArrayKeys && _xt_start_prim_scan(scan, ForwardScanDirection));
 ...
 */
IndexScanDesc
_xtbeginscan(Relation rel, int nkeys, int norderbys)
{
	IndexScanDesc scan;
	BTScanOpaque so;
 ...
 */
void
_xtrescan(IndexScanDesc scan, ScanKey scankey, int nscankeys,
		 ScanKey orderbys, int norderbys)
{
	...
	so->numberOfKeys = 0;		/* until _xt_preprocess_keys sets it */
	so->numArrayKeys = 0;		/* ditto */
}

/*
 *	_xtendscan() -- close down a scan
 */
void
_xtendscan(IndexScanDesc scan)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
...
_xtmarkpos(IndexScanDesc scan)
{
	 ...
	 * before releasing the marked position, _xt_steppage makes a full copy of
 ...
 */
void
_xtrestrpos(IndexScanDesc scan)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	...
	else
	{
		...
		if (BTScanPosIsValid(so->markPos))
		{
			...
			/* Reset the scan's array keys (see _xt_steppage for why) */
			if (so->numArrayKeys)
			{
				_xt_start_array_keys(scan, so->currPos.dir);
				so->needPrimScan = false;
			}
		}
		else
 ...
 */
Size
_xtestimateparallelscan(int nkeys, int norderbys)
{
...
/*
 * btinitparallelscan -- initialize BTParallelScanDesc for parallel _xtree scan
 */
void
_xtinitparallelscan(void *target)
{
 ...
 */
void _xtparallelrescan(IndexScanDesc scan)
{
...
/*
 * _xt_parallel_seize() -- Begin the process of advancing the scan to a new
 *		page.  Other scans must wait until we call _xt_parallel_release()
 *		or _xt_parallel_done().
 *
 ...
 */
bool
_xt_parallel_seize(IndexScanDesc scan, BlockNumber *pageno, bool first)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	...
	*pageno = P_NONE;
	...
	{
		/*
		 * Initialize array related state when called from _xt_first, assuming
...
/*
 * _xt_parallel_release() -- Complete the process of advancing the scan to a
 *		new page.  We now have the new value btps_scanPage; some other backend
 *		can now begin advancing the scan.
 *
 * Callers whose scan uses array keys must save their scan_page argument so
 * that it can be passed to _xt_parallel_primscan_schedule, should caller
 ...
 */
void
_xt_parallel_release(IndexScanDesc scan, BlockNumber scan_page)
{
	ParallelIndexScanDesc parallel_scan = scan->parallel_scan;
	BTParallelScanDesc btscan;
...
/*
 * _xt_parallel_done() -- Mark the parallel scan as complete.
 *
 ...
 */
void
_xt_parallel_done(IndexScanDesc scan)
{
	ParallelIndexScanDesc parallel_scan = scan->parallel_scan;
	BTParallelScanDesc btscan;
...
/*
 * _xt_parallel_primscan_schedule() -- Schedule another primitive index scan.
 *
 * Caller passes the block number most recently passed to _xt_parallel_release
 ...
 */
void
_xt_parallel_primscan_schedule(IndexScanDesc scan, BlockNumber prev_scan_page)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	ParallelIndexScanDesc parallel_scan = scan->parallel_scan;
	BTParallelScanDesc btscan;
 ...
 */
IndexBulkDeleteResult * _xtbulkdelete(IndexVacuumInfo *info, IndexBulkDeleteResult *stats,
			 IndexBulkDeleteCallback callback, void *callback_state)
{
	...
	{
		cycleid = _xt_start_vacuum(rel);

		_xtvacuumscan(info, stats, callback, callback_state, cycleid);
	}
	PG_END_ENSURE_ERROR_CLEANUP(_xt_end_vacuum_callback, PointerGetDatum(rel));
	_xt_end_vacuum(rel);
 ...
 * Result: a palloc'd struct containing statistical info for VACUUM displays.
 */
IndexBulkDeleteResult * _xtvacuumcleanup(IndexVacuumInfo *info, IndexBulkDeleteResult *stats)
{
	 ...
	 * the information used within _xt_vacuum_needs_cleanup() by calling
	 * _xt_set_cleanup_info() below).
	 *
	 ...
	 * of the index).  A call to _xt_vacuum_needs_cleanup() decides it for us
	 * now.
	 */
	if (stats == NULL)
	{
		/* Check if VACUUM operation can entirely avoid btvacuumscan() call */
		if (!_xt_vacuum_needs_cleanup(info->index))
			return NULL;
		 ...
		 */
		stats = (IndexBulkDeleteResult *) palloc0(sizeof(IndexBulkDeleteResult));
		_xtvacuumscan(info, stats, NULL, NULL, 0);
		stats->estimated_count = true;
	}

	/*
	 * Maintain num_delpages value in metapage for _xt_vacuum_needs_cleanup().
	 *
	 ...
	 * greater than 0 only when _xt_pagedel() actually deleted pages during
	 * our call to btvacuumscan().  Even then, _xt_pendingfsm_finalize() must
	...
	_xt_set_cleanup_info(info->index, num_delpages);
 ...
 * btbulkdelete call occurred and _xt_vacuum_needs_cleanup returned true).
 *
			 ...
			 BTCycleId cycleid)
{
	...
	/* Create a temporary memory context to run _xt_pagedel in */
	vstate.pagedelcontext = AllocSetContextCreate(CurrentMemoryContext,
												  "_xt_pagedel",
												  ALLOCSET_DEFAULT_SIZES);

	/* Initialize vstate fields used by _xt_pendingfsm_finalize */
	vstate.bufsize = 0;
	vstate.maxbufsize = 0;
	vstate.pendingpages = NULL;
	vstate.npendingpages = 0;
	/* Consider applying _xt_pendingfsm_finalize optimization */
	_xt_pendingfsm_init(rel, &vstate, (callback == NULL));
	 ...
	 * all-zero page but it hasn't yet been write-locked by _xt_getbuf(). If
	 * we manage to scan such a page here, we'll improperly assume it can be
	 * recycled.  Taking the lock synchronizes things enough to prevent a
	 * problem: either num_pages won't include the new page, or _xt_getbuf
	...
	for (;;)
	{
		...
		{
			_xtvacuumpage(&vstate, scanblkno);
			if (info->report_progress)
	...
	/*
	 * If there were any calls to _xt_pagedel() during scan of the index then
	 ...
	 */
	_xt_pendingfsm_finalize(rel, &vstate);
...
backtrack:
	...
	/*
	 * We can't use _xt_getbuf() here because it always applies
	 * _xt_checkpage(), which will barf on an all-zero page. We want to
	 * recycle all-zero pages, not fail.  Also, we want to use a nondefault
	 * buffer access strategy.
	 */
	buf = ReadBufferExtended(rel, MAIN_FORKNUM, blkno, RBM_NORMAL,
							 info->strategy);
	_xt_lockbuf(rel, buf, BT_READ);
	...
	if (blkno != scanblkno)
	{
		 ...
		 * always marked fully deleted by _xt_pagedel(), barring corruption).
		 */
		 ...
		 * since _xt_pagedel() sometimes deletes the right sibling page of
	...
	else if (P_ISHALFDEAD(opaque))
	{
		...
		/*
		 * _xt_pagedel() will increment both pages_newly_deleted and
	...
	else if (P_ISLEAF(opaque))
	{
		 ...
		 */
		_xt_upgradelockbufcleanup(rel, buf);
		...
		if (callback)
		{
				 ...
				 offnum = OffsetNumberNext(offnum))
			{
				...
				else
				{
					...
					/* Posting list tuple */
					vacposting = _xtreevacuumposting(vstate, itup, offnum,
													&nremaining);
					if (vacposting == NULL)
					{
						/*
		 ...
		 * _xt_delitems_vacuum() call per page, so as to minimize WAL traffic.
		 */
		if (ndeletable > 0 || nupdatable > 0)
		{
			Assert(nhtidsdead >= ndeletable + nupdatable);
			_xt_delitems_vacuum(rel, buf, deletable, ndeletable, updatable,
								nupdatable);
		...
		else
		{
			 ...
			 * don't have any deletions to do.  (If we do, _xt_delitems_vacuum
		 ...
		 * since that would require teaching _xt_pagedel() about backtracking
	...
	if (attempt_pagedel)
	{
		...
		/*
		 * _xt_pagedel maintains the bulk delete stats on our behalf;
		 * pages_newly_deleted and pages_deleted are likely to be incremented
		 * during call
		 */
		Assert(blkno == scanblkno);
		_xt_pagedel(rel, buf, vstate);
 ...
 */
static BTVacuumPosting _xtreevacuumposting(BTVacState *vstate, IndexTuple posting,
 ...
 */
bool _xtcanreturn(Relation index, int attno)
{
	return true;
}

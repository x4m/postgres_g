...
static void _xt_bottomupdel_finish_pending(Page page, BTDedupState state,
										   TM_IndexDeleteOp *delstate);
static bool _xt_do_singleval(Relation rel, Page page, BTDedupState state,
							 OffsetNumber minoff, IndexTuple newitem);
static void _xt_singleval_fillfactor(Page page, BTDedupState state,
									 Size newitemsz);
#ifdef USE_ASSERT_CHECKING
static bool _xt_posting_valid(IndexTuple posting);
#endif
 ...
 *
 * When a call to _xt_bottomupdel_pass() just took place (and failed), our
 ...
 */
void
_xt_dedup_pass(Relation rel, Buffer buf, IndexTuple newitem, Size newitemsz,
			   bool bottomupdedup)
{
	OffsetNumber offnum,
				minoff,
	 ...
	 */
	if (!bottomupdedup)
		singlevalstrat = _xt_do_singleval(rel, page, state, minoff, newitem);
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		if (offnum == minoff)
		{
			 ...
			 */
			_xt_dedup_start_pending(state, itup, offnum);
		}
		else if (state->deduplicate &&
				 _xt_keep_natts_fast(rel, state->base, itup) > nkeyatts &&
				 _xt_dedup_save_htid(state, itup))
		{
			/*
		...
		else
		{
			 ...
			 */
			pagesaving += _xt_dedup_finish_pending(newpage, state);
			...
			if (singlevalstrat)
			{
				...
				if (state->nmaxitems == 5)
					_xt_singleval_fillfactor(page, state, newitemsz);
 ...
 */
bool
_xt_bottomupdel_pass(Relation rel, Buffer buf, Relation heapRel,
					 Size newitemsz)
{
	OffsetNumber offnum,
				minoff,
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		else if (_xt_keep_natts_fast(rel, state->base, itup) > nkeyatts &&
				 _xt_dedup_save_htid(state, itup))
		{
		...
		else
		{
			...
			_xt_bottomupdel_finish_pending(page, state, &delstate);
	...
	_xt_delitems_delete_check(rel, buf, heapRel, &delstate);
 ...
 * actually be rewritten within _xt_dedup_finish_pending() when it turns out
 * that there are duplicates that can be merged into the base tuple.
 */
void
_xt_dedup_start_pending(BTDedupState state, IndexTuple base,
 ...
 */
bool
_xt_dedup_save_htid(BTDedupState state, IndexTuple itup)
{
	int			nhtids;
	ItemPointer htids;
	Size		mergedtupsz;
	 ...
	 *
	 * This calculation needs to match the code used within _xt_form_posting()
	...
	if (mergedtupsz > state->maxpostingsize)
	{
		 ...
		 * deduplication pass, or current pass).  See _xt_do_singleval()
		 * comments.
		 */
		if (state->nhtids > 50)
			state->nmaxitems++;
 ...
 */
Size
_xt_dedup_finish_pending(Page newpage, BTDedupState state)
{
	OffsetNumber tupoff;
	Size		tuplesz;
	Size		spacesaving;
	...
	else
	{
		...
		final = _xt_form_posting(state->base, state->htids, state->nhtids);
 ...
 */
static void
_xt_bottomupdel_finish_pending(Page page, BTDedupState state,
							   TM_IndexDeleteOp *delstate)
{
 ...
 * reaches _xt_singleval_fillfactor(), and so will deliberately leave behind a
 ...
 */
static bool
_xt_do_singleval(Relation rel, Page page, BTDedupState state,
				 OffsetNumber minoff, IndexTuple newitem)
{
	int			nkeyatts = IndexRelationGetNumberOfKeyAttributes(rel);
	ItemId		itemid;
	...
	if (_xt_keep_natts_fast(rel, newitem, itup) > nkeyatts)
	{
 ...
 * plain non-pivot tuples -- see _xt_do_singleval() comments.)
 */
static void
_xt_singleval_fillfactor(Page page, BTDedupState state, Size newitemsz)
{
	Size		leftfree;
	int			reduction;
 ...
 */
IndexTuple
_xt_form_posting(IndexTuple base, ItemPointer htids, int nhtids)
{
	uint32		keysize,
				newsize;
	IndexTuple	itup;
 ...
 */
void
_xt_update_posting(BTVacuumPosting vacposting)
{
	...
	Assert(_xt_posting_valid(origtuple));
	 ...
	 *
	 * This calculation needs to match the code used within _xt_form_posting()
	 * for new posting list tuples.  We avoid calling _xt_form_posting() here
	...
	Assert(nhtids == 1 || _xt_posting_valid(itup));
 ...
 */
IndexTuple
_xt_swap_posting(IndexTuple newitem, IndexTuple oposting, int postingoff)
{
	int			nhtids;
	char	   *replacepos;
	...
	Assert(_xt_posting_valid(oposting));

	/*
	 * The postingoff argument originated as a _xt_binsrch_posting() return
	...
	Assert(_xt_posting_valid(nposting));
...
#ifdef USE_ASSERT_CHECKING
static bool
_xt_posting_valid(IndexTuple posting)
{

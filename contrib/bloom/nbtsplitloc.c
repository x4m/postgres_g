...
typedef struct
{
	...
	/* split point identifying fields (returned by _xt_findsplitloc) */
...
typedef struct
{
	/* context data for _xt_recsplitloc */
...
static void _xt_recsplitloc(FindSplitData *state,
							OffsetNumber firstrightoff, bool newitemonleft,
							int olddataitemstoleft,
							Size firstrightofforigpagetuplesz);
static void _xt_deltasortsplits(FindSplitData *state, double fillfactormult,
								bool usemult);
static int	_xt_splitcmp(const void *arg1, const void *arg2);
static bool _xt_afternewitemoff(FindSplitData *state, OffsetNumber maxoff,
								int leaffillfactor, bool *usemult);
static bool _xt_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid);
static OffsetNumber _xt_bestsplitloc(FindSplitData *state, int perfectpenalty,
									 bool *newitemonleft, FindSplitStrat strategy);
static int	_xt_defaultinterval(FindSplitData *state);
static int	_xt_strategy(FindSplitData *state, SplitPoint *leftpage,
						 SplitPoint *rightpage, FindSplitStrat *strategy);
static void _xt_interval_edges(FindSplitData *state,
							   SplitPoint **leftinterval, SplitPoint **rightinterval);
static inline int _xt_split_penalty(FindSplitData *state, SplitPoint *split);
static inline IndexTuple _xt_split_lastleft(FindSplitData *state,
											SplitPoint *split);
static inline IndexTuple _xt_split_firstright(FindSplitData *state,
											  SplitPoint *split);


/*
 *	_xt_findsplitloc() -- find an appropriate place to split a page.
 *
				 ...
				 bool *newitemonleft)
{
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		 ...
		 * page. (_xt_recsplitloc() will reject the split when there are no
		 * previous items, which we rely on.)
		 */
		if (offnum < newitemoff)
			_xt_recsplitloc(&state, offnum, false, olddataitemstoleft, itemsz);
		else if (offnum > newitemoff)
			_xt_recsplitloc(&state, offnum, true, olddataitemstoleft, itemsz);
		else
		{
			/*
	...
	if (newitemoff > maxoff)
		_xt_recsplitloc(&state, newitemoff, false, olddataitemstotal, 0);
	 ...
	 * considered.  See _xt_defaultinterval().
	 */
	if (!state.is_leaf)
	{
	...
	else if (_xt_afternewitemoff(&state, maxoff, leaffillfactor, &usemult))
	{
		/*
	...
	_xt_deltasortsplits(&state, fillfactormult, usemult);

	/* Determine split interval for default strategy */
	state.interval = _xt_defaultinterval(&state);
	 ...
	 */
	perfectpenalty = _xt_strategy(&state, &leftpage, &rightpage, &strategy);
	 ...
	 */
	firstrightoff = _xt_bestsplitloc(&state, perfectpenalty, newitemonleft,
									 strategy);
	pfree(state.splits);
 ...
 */
static void
_xt_recsplitloc(FindSplitData *state,
 ...
 */
static void
_xt_deltasortsplits(FindSplitData *state, double fillfactormult,
					bool usemult)
{
	for (int i = 0; i < state->nsplits; i++)
	{
	...
	qsort(state->splits, state->nsplits, sizeof(SplitPoint), _xt_splitcmp);
}

/*
 * qsort-style comparator used by _xt_deltasortsplits()
 */
static int
_xt_splitcmp(const void *arg1, const void *arg2)
{
 ...
 */
static bool
_xt_afternewitemoff(FindSplitData *state, OffsetNumber maxoff,
					int leaffillfactor, bool *usemult)
{
	int16		nkeyatts;
	ItemId		itemid;
	...
	if (state->newitemoff > maxoff)
	{
		...
		keepnatts = _xt_keep_natts_fast(state->rel, tup, state->newitem);
	...
	if (BTreeTupleIsPosting(tup) ||
		!_xt_adjacenthtid(&tup->t_tid, &state->newitem->t_tid))
		return false;
 ...
 */
static bool
_xt_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid)
{
	BlockNumber lowblk,
				highblk;
 ...
 * splitting a leaf page or an internal page.  See _xt_split_penalty() for
 * details.
 *
 ...
 */
static OffsetNumber
_xt_bestsplitloc(FindSplitData *state, int perfectpenalty,
				 bool *newitemonleft, FindSplitStrat strategy)
{
	int			bestpenalty,
				lowsplit;
	...
	for (int i = lowsplit; i < highsplit; i++)
	{
		...
		penalty = _xt_split_penalty(state, state->splits + i);
 ...
 * interval is applied within _xt_bestsplitloc().
 *
 ...
 * which case the interval also changes -- see _xt_strategy()).
 *
 ...
 */
static int
_xt_defaultinterval(FindSplitData *state)
{
 ...
 * passed to _xt_bestsplitloc() as a final constraint on how far caller is
 * willing to go to avoid appending a heap TID when using the many duplicates
 * strategy (it also saves _xt_bestsplitloc() useless cycles).
 */
static int
_xt_strategy(FindSplitData *state, SplitPoint *leftpage,
			 SplitPoint *rightpage, FindSplitStrat *strategy)
{
	IndexTuple	leftmost,
				rightmost;
	...
	*strategy = SPLIT_DEFAULT;
	 ...
	 */
	_xt_interval_edges(state, &leftinterval, &rightinterval);
	leftmost = _xt_split_lastleft(state, leftinterval);
	rightmost = _xt_split_firstright(state, rightinterval);
	 ...
	 */
	perfectpenalty = _xt_keep_natts_fast(state->rel, leftmost, rightmost);
	 ...
	 */
	leftmost = _xt_split_lastleft(state, leftpage);
	rightmost = _xt_split_firstright(state, rightpage);
	...
	if (perfectpenalty <= indnkeyatts)
	{
		*strategy = SPLIT_MANY_DUPLICATES;
		 ...
		 * before a final decision is made, within _xt_bestsplitloc().
		 */
		return indnkeyatts;
	}
	...
	else
	{
		...
		perfectpenalty = _xt_keep_natts_fast(state->rel, hikey,
											 state->newitem);
		if (perfectpenalty <= indnkeyatts)
			*strategy = SPLIT_SINGLE_VALUE;
		else
 ...
 */
static void
_xt_interval_edges(FindSplitData *state, SplitPoint **leftinterval,
 ...
 */
static inline int
_xt_split_penalty(FindSplitData *state, SplitPoint *split)
{
	IndexTuple	lastleft;
	...
	lastleft = _xt_split_lastleft(state, split);
	firstright = _xt_split_firstright(state, split);

	return _xt_keep_natts_fast(state->rel, lastleft, firstright);
}
...
_xt_split_lastleft(FindSplitData *state, SplitPoint *split)
{
	ItemId		itemid;
 ...
 */
static inline IndexTuple
_xt_split_firstright(FindSplitData *state, SplitPoint *split)
{
	ItemId		itemid;

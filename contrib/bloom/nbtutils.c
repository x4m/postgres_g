...
static void _xt_setup_array_cmp(IndexScanDesc scan, ScanKey skey, Oid elemtype,
								FmgrInfo *orderproc, FmgrInfo **sortprocp);
static Datum _xt_find_extreme_element(IndexScanDesc scan, ScanKey skey,
									  Oid elemtype, StrategyNumber strat,
									  Datum *elems, int nelems);
static int	_xt_sort_array_elements(ScanKey skey, FmgrInfo *sortproc,
									bool reverse, Datum *elems, int nelems);
static bool _xt_merge_arrays(IndexScanDesc scan, ScanKey skey,
...
static bool _xt_compare_array_scankey_args(IndexScanDesc scan,
										   ScanKey arraysk, ScanKey skey,
										   FmgrInfo *orderproc, BTArrayKeyInfo *array,
										   bool *qual_ok);
static ScanKey _xt_preprocess_array_keys(IndexScanDesc scan);
static void _xt_preprocess_array_keys_final(IndexScanDesc scan, int *keyDataMap);
static int	_xt_compare_array_elements(const void *a, const void *b, void *arg);
static inline int32 _xt_compare_array_skey(FmgrInfo *orderproc,
										   Datum tupdatum, bool tupnull,
										   Datum arrdatum, ScanKey cur);
static int	_xt_binsrch_array_skey(FmgrInfo *orderproc,
								   bool cur_elem_trig, ScanDirection dir,
								   Datum tupdatum, bool tupnull,
								   BTArrayKeyInfo *array, ScanKey cur,
								   int32 *set_elem_result);
static bool _xt_advance_array_keys_increment(IndexScanDesc scan, ScanDirection dir);
static void _xt_rewind_nonrequired_arrays(IndexScanDesc scan, ScanDirection dir);
static bool _xt_tuple_before_array_skeys(IndexScanDesc scan, ScanDirection dir,
										 IndexTuple tuple, TupleDesc tupdesc, int tupnatts,
										 bool readpagetup, int sktrig, bool *scanBehind);
static bool _xt_advance_array_keys(IndexScanDesc scan, BTReadPageState *pstate,
								   IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
								   int sktrig, bool sktrig_required);
#ifdef USE_ASSERT_CHECKING
static bool _xt_verify_arrays_xt_first(IndexScanDesc scan, ScanDirection dir);
static bool _xt_verify_keys_with_arraykeys(IndexScanDesc scan);
#endif
static bool _xt_compare_scankey_args(IndexScanDesc scan, ScanKey op,
									 ScanKey leftarg, ScanKey rightarg,
									 BTArrayKeyInfo *array, FmgrInfo *orderproc,
									 bool *result);
static bool _xt_fix_scankey_strategy(ScanKey skey, int16 *indoption);
static void _xt_mark_scankey_required(ScanKey skey);
static bool _xt_check_compare(IndexScanDesc scan, ScanDirection dir,
							  IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
							  bool advancenonrequired, bool prechecked, bool firstmatch,
							  bool *continuescan, int *ikey);
static bool _xt_check_rowcompare(ScanKey skey,
								 IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
								 ScanDirection dir, bool *continuescan);
static void _xt_checkkeys_look_ahead(IndexScanDesc scan, BTReadPageState *pstate,
									 int tupnatts, TupleDesc tupdesc);
static int	_xt_keep_natts(Relation rel, IndexTuple lastleft,
 ...
 *
 *		The result is intended for use with _xt_compare() and _xt_truncate().
 ...
 *		for _xt_truncate()) can pass a NULL index tuple.  The scankey will
 ...
 *		is passed; CREATE INDEX callers call _xt_allequalimage() to set the
 *		field themselves.
 */
BTScanInsert
_xt_mkscankey(Relation rel, IndexTuple itup)
{
	BTScanInsert key;
	ScanKey		skey;
	TupleDesc	itupdesc;
	...
	if (itup)
		_xt_metaversion(rel, &key->heapkeyspace, &key->allequalimage);
	else
	{
...
/*
 * free a retracement stack made by _xt_search.
 */
void
_xt_freestack(BTStack stack)
{
	BTStack		ostack;
...
/*
 *	_xt_preprocess_array_keys() -- Preprocess SK_SEARCHARRAY scan keys
 *
 ...
 *
 * It is convenient for _xt_preprocess_keys caller to have to deal with no
 ...
 * have array keys should call _xt_preprocess_array_keys_final when standard
 ...
 */
static ScanKey
_xt_preprocess_array_keys(IndexScanDesc scan)
{
	...
	/* Allocate space for ORDER procs used to help _xt_checkkeys */
	...
	for (int i = 0; i < numberOfKeys; i++)
	{
		...
		switch (cur->sk_strategy)
		{
			...
			case BTLessEqualStrategyNumber:
				cur->sk_argument =
					_xt_find_extreme_element(scan, cur, elemtype,
		 ...
		 */
		_xt_setup_array_cmp(scan, cur, elemtype,
		...
		num_elems = _xt_sort_array_elements(cur, sortprocp, reverse,
											elem_values, num_nonnulls);
		...
		if (origarrayatt == cur->sk_attno)
		{
			...
			Assert(arrayKeyData[orig->scan_key].sk_collation ==
				   cur->sk_collation);
			if (_xt_merge_arrays(scan, cur, sortprocp, reverse,
								 ...
								 elem_values, num_elems))
			{
				...
				/*
				 * Indicate to _xt_preprocess_keys caller that it must ignore
				 * this scan key
				 */
		 ...
		 *
		 * Note: _xt_preprocess_array_keys_final will fix-up each array's
...
/*
 *	_xt_preprocess_array_keys_final() -- fix up array scan key references
 *
 * When _xt_preprocess_array_keys performed initial array preprocessing, it
 ...
 */
static void
_xt_preprocess_array_keys_final(IndexScanDesc scan, int *keyDataMap)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	Relation	rel = scan->indexRelation;
	int			arrayidx = 0;
	...
	/*
	 * Nothing for us to do when _xt_preprocess_array_keys only had to deal
	...
	for (int output_ikey = 0; output_ikey < so->numberOfKeys; output_ikey++)
	{
		...
		if (!(outkey->sk_flags & SK_SEARCHARRAY))
		{
			...
			_xt_setup_array_cmp(scan, outkey, elemtype,
...
/*
 * _xt_setup_array_cmp() -- Set up array comparison functions
 *
 ...
 * See _xt_advance_array_keys for an explanation of why it'll need to treat
 ...
 */
static void
_xt_setup_array_cmp(IndexScanDesc scan, ScanKey skey, Oid elemtype,
					FmgrInfo *orderproc, FmgrInfo **sortprocp)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	Relation	rel = scan->indexRelation;
	 ...
	 * incomplete, but only in cases where it's quite likely that _xt_first
...
/*
 * _xt_find_extreme_element() -- get least or greatest array element
 *
 ...
 */
static Datum
_xt_find_extreme_element(IndexScanDesc scan, ScanKey skey, Oid elemtype,
...
/*
 * _xt_sort_array_elements() -- sort and de-dup array elements
 *
 ...
 */
static int
_xt_sort_array_elements(ScanKey skey, FmgrInfo *sortproc, bool reverse,
						Datum *elems, int nelems)
{
	...
	qsort_arg(elems, nelems, sizeof(Datum),
			  _xt_compare_array_elements, &cxt);
...
/*
 * _xt_merge_arrays() -- merge next array's elements into an original array
 *
 ...
 * _xt_sort_array_elements.  sortproc is the same-type ORDER proc that was
 ...
 */
static bool
_xt_merge_arrays(IndexScanDesc scan, ScanKey skey, FmgrInfo *sortproc,
				 ...
				 Datum *elems_next, int nelems_next)
{
	...
	for (int i = 0, j = 0; i < nelems_orig_start && j < nelems_next;)
	{
		Datum	   *oelem = elems_orig + i,
				   *nelem = elems_next + j;
		int			res = _xt_compare_array_elements(oelem, nelem, &cxt);
 ...
 */
static bool
_xt_compare_array_scankey_args(IndexScanDesc scan, ScanKey arraysk, ScanKey skey,
							   FmgrInfo *orderproc, BTArrayKeyInfo *array,
							   bool *qual_ok)
{
	Relation	rel = scan->indexRelation;
	...
	/*
	 * _xt_binsrch_array_skey searches an array for the entry best matching a
	 ...
	 * _xt_binsrch_array_skey applies the correct comparator.
	 *
	...
	if (skey->sk_subtype != opcintype && skey->sk_subtype != InvalidOid)
	{
		 ...
		 *
		 * Scalar scan key's argument will be passed to _xt_compare_array_skey
	...
	matchelem = _xt_binsrch_array_skey(orderprocp, false,
 ...
 */
static int
_xt_compare_array_elements(const void *a, const void *b, void *arg)
{
...
/*
 * _xt_compare_array_skey() -- apply array comparison function
 *
 ...
 *			>0 if tupdatum > arrdatum.
 *
 * This is essentially the same interface as _xt_compare: both functions
 * compare the value that they're searching for to a binary search pivot.
 * However, unlike _xt_compare, this function's "tuple argument" comes first,
 * while its "array/scankey argument" comes second.
*/
static inline int32
_xt_compare_array_skey(FmgrInfo *orderproc,
					   Datum tupdatum, bool tupnull,
					   Datum arrdatum, ScanKey cur)
{
	int32		result = 0;
	...
	{
		/*
		 * Like _xt_compare, we need to be careful of cross-type comparisons,
		 ...
		 *
		 * _xt_compare does it the wrong way around (flip when *ASC*) in order
...
/*
 * _xt_binsrch_array_skey() -- Binary search for next matching array key
 *
 ...
 *
 * Also sets *set_elem_result to the result _xt_compare_array_skey returned
 ...
 */
static int
_xt_binsrch_array_skey(FmgrInfo *orderproc,
					   bool cur_elem_trig, ScanDirection dir,
					   Datum tupdatum, bool tupnull,
					   BTArrayKeyInfo *array, ScanKey cur,
					   int32 *set_elem_result)
{
	...
	if (cur_elem_trig)
	{
		 ...
		 * (_xt_checkkeys must have called _xt_tuple_before_array_skeys, which
		...
		if (ScanDirectionIsForward(dir))
		{
			...
			if (high_elem >= low_elem)
			{
				...
				result = _xt_compare_array_skey(orderproc, tupdatum, tupnull,
												arrdatum, cur);
...
/*
 * _xt_start_array_keys() -- Initialize array keys at start of a scan
 *
 * Set up the cur_elem counters and fill in the first sk_argument value for
 * each array scankey.
 */
void
_xt_start_array_keys(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	int			i;
...
/*
 * _xt_advance_array_keys_increment() -- Advance to next set of array elements
 *
 ...
 */
static bool
_xt_advance_array_keys_increment(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	 ...
	 */
	_xt_start_array_keys(scan, -dir);

	return false;
}

/*
 * _xt_rewind_nonrequired_arrays() -- Rewind non-required arrays
 *
 * Called when _xt_advance_array_keys decides to start a new primitive index
 * scan on the basis of the current scan position being before the position
 * that _xt_first is capable of repositioning the scan to by applying an
 ...
 * first descent of the tree by _xt_first.  Later on, there could also be a
 * second descent, that places the scan right before tuples >= "(200, 3, 5)".
 * _xt_first must never be allowed to build an insertion scan key whose "c"
 ...
 * Not all callers are at risk of _xt_first using a non-required array like
 ...
 *
 * Note: _xt_verify_arrays_xt_first is called by an assertion to enforce that
 * everybody got this right.
 */
static void
_xt_rewind_nonrequired_arrays(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	int			arrayidx = 0;
...
/*
 * _xt_tuple_before_array_skeys() -- too early to advance required arrays?
 *
 ...
 * scan's current _xt_readpage-wise tuple.
 *
 * readpagetup callers must only call here when _xt_check_compare already set
 * continuescan=false.  We help these callers deal with _xt_check_compare's
 ...
 * a _xt_check_compare-set sktrig value that indicates which scan key
 ...
 * that were already deemed to have been satisfied inside _xt_check_compare.
 *
 ...
 * inequality strategy scan key wasn't satisfied in _xt_check_compare when we
 * return true.  In fact, we automatically return false when passed such an
 * inequality sktrig by readpagetup callers -- _xt_check_compare's initial
 ...
 */
static bool
_xt_tuple_before_array_skeys(IndexScanDesc scan, ScanDirection dir,
							 IndexTuple tuple, TupleDesc tupdesc, int tupnatts,
							 bool readpagetup, int sktrig, bool *scanBehind)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	...
	for (int ikey = sktrig; ikey < so->numberOfKeys; ikey++)
	{
		 ...
		 * _xt_advance_array_keys caller requires that we track *scanBehind
		...
		/*
		 * Deal with inequality strategy scan keys that _xt_check_compare set
		 * continuescan=false for
		 */
		if (cur->sk_strategy != BTEqualStrategyNumber)
		{
			/*
			 * When _xt_check_compare indicated that a required inequality
			 * scan key wasn't satisfied, there's no need to verify anything;
			 * caller always calls _xt_advance_array_keys with this sktrig.
			 */
			if (readpagetup)
				return false;
		...
		result = _xt_compare_array_skey(&so->orderProcs[ikey],
										tupdatum, tupnull,
										cur->sk_argument, cur);
...
/*
 * _xt_start_prim_scan() -- start scheduled primitive index scan?
 *
 * Returns true if _xt_checkkeys scheduled another primitive index scan, just
 ...
 * after _xt_first or _xt_next return false.
 */
bool
_xt_start_prim_scan(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	...
	/*
	 * Array keys are advanced within _xt_checkkeys when the scan reaches the
	 ...
	 *
	 * _xt_checkkeys sets a simple flag variable to schedule another primitive
	 * index scan.  The flag tells us what to do.
	 *
	 * We cannot rely on _xt_first always reaching _xt_checkkeys.  There are
	 * various cases where that won't happen.  For example, if the index is
	 * completely empty, then _xt_first won't call _xt_readpage/_xt_checkkeys.
	 * We also don't expect a call to _xt_checkkeys during searches for a
	 ...
	 * It's up to code under the control of _xt_first to always set the flag
	 ...
	 * leaf root page).  If _xt_checkkeys cannot be reached by a primitive
	 ...
	 * cases, but that's the correct behavior (even _xt_advance_array_keys
	 * won't always advance the arrays at the point they become "exhausted").
	 */
	if (so->needPrimScan)
	{
		Assert(_xt_verify_arrays_xt_first(scan, dir));

		/*
		 * Flag was set -- must call _xt_first again, which will reset the
	...
	if (scan->parallel_scan != NULL)
		_xt_parallel_done(scan);

	return false;
}

/*
 * _xt_advance_array_keys() -- Advance array elements using a tuple
 *
 ...
 * All later _xt_check_compare calls also use the same new qual that was first
 * used here (at least until the next call here advances the keys once again).
 * It's convenient to structure _xt_check_compare rechecks of caller's tuple
 * (using the new qual) as one the steps of advancing the scan's array keys,
 * so this function works as a wrapper around _xt_check_compare.
 *
 * Like _xt_check_compare, we'll set pstate.continuescan on behalf of the
 * caller, and return a boolean indicating if caller's tuple satisfies the
 * scan's new qual.  But unlike _xt_check_compare, we set so->needPrimScan
 ...
 *
 * Caller must use _xt_tuple_before_array_skeys to determine if the current
 ...
 * of stopping the scan).  No call to _xt_tuple_before_array_skeys is required
 ...
 * do this for inequalities that _xt_check_compare set continuescan=false for.
 * They'll advance the array keys here, just like any other scan key that
 * _xt_check_compare stops on.  (This can even happen _after_ we advance the
 * array keys, in which case we'll advance the array keys a second time.  That
 * way _xt_checkkeys caller always has its required arrays advance to the
 * maximum possible extent that its tuple will allow.)
 */
static bool
_xt_advance_array_keys(IndexScanDesc scan, BTReadPageState *pstate,
					   IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
					   int sktrig, bool sktrig_required)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	...
	if (sktrig_required)
	{
		 ...
		 */
		Assert(!_xt_tuple_before_array_skeys(scan, dir, tuple, tupdesc,
	...
	Assert(_xt_verify_keys_with_arraykeys(scan));
	...
	for (int ikey = 0; ikey < so->numberOfKeys; ikey++)
	{
		...
		if (cur->sk_flags & (SK_BT_REQFWD | SK_BT_REQBKWD))
		{
			...
			if (cur->sk_attno > tupnatts)
			{
				/* Set this just like _xt_tuple_before_array_skeys */
				Assert(sktrig < ikey);
				so->scanBehind = true;
			}
		}
		 ...
		 * _xt_check_compare indicated triggered array advancement, if any.
		 *
		 ...
		 * optimization; _xt_tuple_before_array_skeys already confirmed that
		 ...
		 * on _xt_check_compare like this; we aren't capable of directly
		 ...
		 * wasn't the one that _xt_check_compare stopped on, though.
		 *
		 * Note: if our later call to _xt_check_compare (to recheck caller's
		...
		if (array)
		{
			 ...
			 */
			set_elem = _xt_binsrch_array_skey(&so->orderProcs[ikey],
											  cur_elem_trig, dir,
		 ...
		 * this array's set_elem" by calling _xt_advance_array_keys_increment.
		 ...
		 * be eliminated by _xt_preprocess_keys.  It won't matter if some of
	 ...
	 */
	if (beyond_end_advance && !_xt_advance_array_keys_increment(scan, dir))
		goto end_toplevel_scan;

	Assert(_xt_verify_keys_with_arraykeys(scan));

	/*
	 * Does tuple now satisfy our new qual?  Recheck with _xt_check_compare.
	 *
	 ...
	 * will still require a recheck call to _xt_check_compare.  They'll still
	 ...
	 * _xt_check_compare call that used the old/original qual.)
	 *
	 ...
	 * satisfies the new qual, so it's safe to just skip the _xt_check_compare
		...
		(!sktrig_required && all_satisfied))
	{
		...
		/* Recheck _xt_check_compare on behalf of caller */
		if (_xt_check_compare(scan, dir, tuple, tupnatts, tupdesc,
							  false, false, false,
		 ...
		 *
		 * It's possible that our _xt_check_compare call indicated that the
		 ...
		 *
		 * We make a strong, general guarantee that every _xt_checkkeys call
		...
		if (unlikely(!continuescan))
		{
			...
			satisfied = _xt_advance_array_keys(scan, pstate, tuple, tupnatts,
	 ...
	 * for _xt_check_compare to behave as if they are required in the current
	 * scan direction to deal with NULLs.  We'll account for that separately.)
	 */
	Assert(_xt_tuple_before_array_skeys(scan, dir, tuple, tupdesc, tupnatts,
										false, 0, NULL) ==
	 ...
	 * it'll be set during this call to _xt_tuple_before_array_skeys.  Either
	 * way, it'll be set correctly (for the whole page) after this point.
	 */
	if (!all_required_satisfied && pstate->finaltup &&
		_xt_tuple_before_array_skeys(scan, dir, pstate->finaltup, tupdesc,
	 ...
	 *
	 * There is a chance that _xt_checkkeys (which checks so->scanBehind) will
	...
	if (so->scanBehind && has_required_opposite_direction_only)
	{
		 ...
		 *
		 * _xt_check_compare simply won't stop the scan for a scan key that's
		 ...
		 * scan will allow _xt_first to skip ahead by a great many leaf pages
	 ...
	 * _xt_first could skip straight to.  Groveling through all those pages
	 * will always give correct answers, but it can be very inefficient.  We
	 * must avoid needlessly scanning extra pages.
	 *
	 * Separately, it's possible that _xt_check_compare set continuescan=false
	 * for a scan key that's required in the opposite direction only.  This is
	 * a special case, that happens only when _xt_check_compare sees that the
	 ...
	 * (_xt_first is expected to skip over the group of NULLs by applying a
	 ...
	 * also before the _xt_first-wise start of tuples for our new qual.  That
		...
		(all_required_satisfied || oppodir_inequality_sktrig))
	{
		 ...
		 * cannot reuse work from caller's earlier _xt_check_compare call.
		 *
		 * Flip the scan direction when calling _xt_check_compare this time,
		...
		opsktrig = 0;
		flipped = -dir;
		_xt_check_compare(scan, flipped,
		 ...
		 * _xt_check_compare not caring about the direction that inequalities
					  ...
					  (oppodir_inequality_sktrig && opsktrig >= sktrig))))
		{
			 ...
			 */
			_xt_rewind_nonrequired_arrays(scan, dir);
	 ...
	 * search of the page, implemented by calling _xt_check_compare and then
	 * _xt_tuple_before_array_skeys for each tuple.
	 *
	 ...
	 */
	pstate->continuescan = true;	/* Override _xt_check_compare */
	so->needPrimScan = false;	/* _xt_readpage has more tuples to check */
...
new_prim_scan:
	 ...
	 */
	pstate->continuescan = false;	/* Tell _xt_readpage we're done... */
	so->needPrimScan = true;	/* ...but call _xt_first again */

	if (scan->parallel_scan)
		_xt_parallel_primscan_schedule(scan, pstate->prev_scan_page);
...
end_toplevel_scan:
	 ...
	 */
	pstate->continuescan = false;	/* Tell _xt_readpage we're done... */
	so->needPrimScan = false;	/* ...don't call _xt_first again, though */
...
/*
 *	_xt_preprocess_keys() -- Preprocess scan keys
 *
 ...
 * respectively.  _xt_checkkeys uses these flags.  For example, if the quals
 * are "x = 1 AND y < 4 AND z < 5", then _xt_checkkeys will reject a tuple
 ...
 * _xt_first will arbitrarily pick one of the keys to do the initial
 ...
 */
void
_xt_preprocess_keys(IndexScanDesc scan)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	int			numberOfKeys = scan->numberOfKeys;
	...
	arrayKeyData = _xt_preprocess_array_keys(scan);
	if (!so->qual_ok)
	{
	 ...
	 * as our input if _xt_preprocess_array_keys just allocated it, else just
	 * use scan->keyData[]
	 */
	if (arrayKeyData)
	{
	...
	if (numberOfKeys == 1)
	{
		...
		if (!_xt_fix_scankey_strategy(cur, indoption))
			so->qual_ok = false;
		memcpy(outkeys, cur, sizeof(ScanKeyData));
		so->numberOfKeys = 1;
		/* We can mark the qual as required if it's for first index col */
		if (cur->sk_attno == 1)
			_xt_mark_scankey_required(outkeys);
		if (arrayKeyData)
		{
			/*
			 * Don't call _xt_preprocess_array_keys_final in this fast path
	...
	for (i = 0;; cur++, i++)
	{
		...
		if (i == numberOfKeys || cur->sk_attno != attno)
		{
			...
			if (xform[BTEqualStrategyNumber - 1].skey)
			{
				...
				for (j = BTMaxStrategyNumber; --j >= 0;)
				{
					...
					if (_xt_compare_scankey_args(scan, chk, eq, chk,
												 array, orderproc,
												 &test_result))
					{
						if (!test_result)
				...
				&& xform[BTLessEqualStrategyNumber - 1].skey)
			{
				...
				if (_xt_compare_scankey_args(scan, le, lt, le, NULL, NULL,
											 &test_result))
				{
					if (test_result)
				...
				&& xform[BTGreaterEqualStrategyNumber - 1].skey)
			{
				...
				if (_xt_compare_scankey_args(scan, ge, gt, ge, NULL, NULL,
											 &test_result))
				{
					if (test_result)
			...
			for (j = BTMaxStrategyNumber; --j >= 0;)
			{
				if (xform[j].skey)
				{
					...
					if (priorNumberOfEqualCols == attno - 1)
						_xt_mark_scankey_required(outkey);
				}
			}
		...
		{
			/* _xt_preprocess_array_keys marked this array key redundant */
			Assert(arrayKeyData);
			Assert(cur->sk_flags & SK_SEARCHARRAY);
			continue;
		}
		...
		{
			/* _xt_preprocess_array_keys kept this array key */
			Assert(arrayKeyData);
			arrayidx++;
		}
		...
		else
		{
			...
			if (j == (BTEqualStrategyNumber - 1) && arrayKeyData)
			{
				 ...
				 * matter, since _xt_compare_scankey_args is aware that two
				 * SEARCHARRAY scan keys mean that _xt_preprocess_array_keys
				 * failed to eliminate redundant arrays through array merging.
				 * _xt_compare_scankey_args just returns false when it sees
				 * this; it won't even try to examine either array.
				 */
			}

			if (_xt_compare_scankey_args(scan, cur, cur, xform[j].skey,
										 array, orderproc, &test_result))
			{
				...
				if (test_result)
				{
					...
					else
					{
						 ...
						 * scan key.  _xt_compare_scankey_args expects us to
			...
			else
			{
				 ...
				 * even with incomplete opfamilies.  _xt_advance_array_keys
				 * depends on this.
				 */
				ScanKey		outkey = &outkeys[new_numberOfKeys++];
	 ...
	 */
	if (arrayKeyData)
		_xt_preprocess_array_keys_final(scan, keyDataMap);
 ...
 * _xt_start_prim_scan is about to start a just-scheduled new primitive scan.
 *
 ...
 * See _xt_rewind_nonrequired_arrays comments for an explanation.
 */
static bool
_xt_verify_arrays_xt_first(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	int			arrayidx = 0;
	...
	return _xt_verify_keys_with_arraykeys(scan);
}
 ...
 */
static bool
_xt_verify_keys_with_arraykeys(IndexScanDesc scan)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
 ...
 * opfamily (_xt_preprocess_array_keys will merge array keys together to make
 * sure of that).
 *
 ...
 */
static bool
_xt_compare_scankey_args(IndexScanDesc scan, ScanKey op,
						 ScanKey leftarg, ScanKey rightarg,
						 BTArrayKeyInfo *array, FmgrInfo *orderproc,
						 bool *result)
{
	...
	if (array)
	{
		...
		/*
		 * _xt_preprocess_array_keys is responsible for merging together array
		 ...
		 */
		if (leftarray)
			return _xt_compare_array_scankey_args(scan, leftarg, rightarg,
												  orderproc, array, result);
		else if (rightarray)
			return _xt_compare_array_scankey_args(scan, rightarg, leftarg,
												  orderproc, array, result);
	 ...
	 * operators.)
	 *
	 * If the sk_strategy was flipped by _xt_fix_scankey_strategy, we have to
 ...
 */
static bool
_xt_fix_scankey_strategy(ScanKey skey, int16 *indoption)
{
	int			addflags;
 ...
 */
static void
_xt_mark_scankey_required(ScanKey skey)
{
	int			addflags;
 ...
 * _xt_preprocess_keys(), above, about how this is done.
 *
 ...
 */
bool
_xt_checkkeys(IndexScanDesc scan, BTReadPageState *pstate, bool arrayKeys,
			  IndexTuple tuple, int tupnatts)
{
	...
	res = _xt_check_compare(scan, dir, tuple, tupnatts, tupdesc,
	...
	if (pstate->prechecked || pstate->firstmatch)
	{
		 ...
		 */
		Assert(res == _xt_check_compare(scan, dir, tuple, tupnatts, tupdesc,
										false, false, false,
										&dcontinuescan, &dikey));
	...
	/*
	 * Only one _xt_check_compare call is required in the common case where
	 * there are no equality strategy array scan keys.  Otherwise we can only
	 * accept _xt_check_compare's answer unreservedly when it didn't set
	...
	/*
	 * _xt_check_compare call set continuescan=false in the presence of
	 ...
	 */
	if (_xt_tuple_before_array_skeys(scan, dir, tuple, tupdesc, tupnatts, true,
									 ikey, NULL))
	{
		/*
		 ...
		 *
		 * _xt_advance_array_keys occasionally sets so->scanBehind to signal
		...
		{
			/* Override _xt_check_compare, continue primitive scan */
			pstate->continuescan = true;
			...
			if (pstate->rechecks >= LOOK_AHEAD_REQUIRED_RECHECKS)
			{
				...
				_xt_checkkeys_look_ahead(scan, pstate, tupnatts, tupdesc);

				/*
				 * Might have set pstate.skip to a later page offset.  When
				 * that happens then _xt_readpage caller will inexpensively
	 ...
	 * the scan.
	 */
	return _xt_advance_array_keys(scan, pstate, tuple, tupnatts, tupdesc,
								  ikey, true);
}
 ...
 * array keys).
 *
 * This is a subroutine for _xt_checkkeys.  We provisionally assume that
 ...
 * This allows _xt_advance_array_keys caller to avoid infinite recursion.
 */
static bool
_xt_check_compare(IndexScanDesc scan, ScanDirection dir,
				  IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
				  bool advancenonrequired, bool prechecked, bool firstmatch,
				  bool *continuescan, int *ikey)
{
	...
	*continuescan = true;		/* default assumption */
	...
	for (; *ikey < so->numberOfKeys; (*ikey)++)
	{
		...
		{
			if (_xt_check_rowcompare(key, tuple, tupnatts, tupdesc, dir,
									 continuescan))
				continue;
			return false;
		}
		...
		if (isNull)
		{
			if (key->sk_flags & SK_BT_NULLS_FIRST)
			{
				 ...
				 * (_xt_advance_array_keys also relies on this behavior during
				 * forward scans.)
				 */
											...
											datum, key->sk_argument)))
		{
			 ...
			 * initial positioning in _xt_first() when they are available. See
			 * comments in _xt_first().
			 */
			if (requiredSameDir)
				*continuescan = false;
			 ...
			 */
				...
				return _xt_advance_array_keys(scan, NULL, tuple, tupnatts,
											  tupdesc, *ikey, false);
 ...
 * to pass the qual.
 *
 * This is a subroutine for _xt_checkkeys/_xt_check_compare.
 */
static bool
_xt_check_rowcompare(ScanKey skey, IndexTuple tuple, int tupnatts,
 ...
 *
 * This is a subroutine for _xt_checkkeys.  Called when _xt_readpage's linear
 ...
 * instructs _xt_readpage to skip ahead to that tuple next (could be past the
 ...
 */
static void
_xt_checkkeys_look_ahead(IndexScanDesc scan, BTReadPageState *pstate,
						 int tupnatts, TupleDesc tupdesc)
{
	ScanDirection dir = pstate->dir;
	 ...
	 * allows _xt_readpage to skip over more tuples
	 */
	...
	if (_xt_tuple_before_array_skeys(scan, dir, ahead, tupdesc, tupnatts,
									 false, 0, NULL))
	{
		/*
		 * Success -- instruct _xt_readpage to skip ahead to very next tuple
...
/*
 * _xt_killitems - set LP_DEAD state for items an indexscan caller has
 * told us were killed
 *
 ...
 */
void
_xt_killitems(IndexScanDesc scan)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	Page		page;
	BTPageOpaque opaque;
	...
	if (BTScanPosIsPinned(so->currPos))
	{
		 ...
		 */
		droppedpin = false;
		_xt_lockbuf(scan->indexRelation, so->currPos.buf, BT_READ);
	...
	else
	{
		...
		buf = _xt_getbuf(scan->indexRelation, so->currPos.currPage, BT_READ);
		...
		else
		{
			...
			_xt_relbuf(scan->indexRelation, buf);
			return;
		}
	}
	...
	_xt_unlockbuf(scan->indexRelation, so->currPos.buf);
}
...
/*
 * _xt_vacuum_cycleid --- get the active vacuum cycle ID for an index,
 ...
 */
BTCycleId
_xt_vacuum_cycleid(Relation rel)
{
	return 0;
	BTCycleId	result = 0;
	int			i;
	...
	if (btvacinfo == NULL)
		btvacinfo = palloc(sizeof(*btvacinfo));
...
/*
 * _xt_start_vacuum --- assign a cycle ID to a just-starting VACUUM operation
 *
 * Note: the caller must guarantee that it will eventually call
 * _xt_end_vacuum, else we'll permanently leak an array slot.  To ensure
 * that this happens even in elog(FATAL) scenarios, the appropriate coding
 * is not just a PG_TRY, but
 *		PG_ENSURE_ERROR_CLEANUP(_xt_end_vacuum_callback, PointerGetDatum(rel))
 */
BTCycleId
_xt_start_vacuum(Relation rel)
{
	BTCycleId	result;
	int			i;
	BTOneVacInfo *vac;
	...
	for (i = 0; i < btvacinfo->num_vacuums; i++)
	{
			...
			vac->relid.dbId == rel->rd_lockInfo.lockRelId.dbId)
		{
			 ...
			 * we expect _xt_end_vacuum() to be called before transaction
...
/*
 * _xt_end_vacuum --- mark a btree VACUUM operation as done
 *
...
/*
 * _xt_end_vacuum wrapped as an on_shmem_exit callback function
 */
void
_xt_end_vacuum_callback(int code, Datum arg)
{
	_xt_end_vacuum((Relation) DatumGetPointer(arg));
}
...
BTreeShmemInit(void)
{
	...
	btvacinfo = (BTVacInfo *) ShmemInitStruct("xTree Vacuum State",
											  BTreeShmemSize(),
											  &found);
...
bytea *
_xtoptions(Datum reloptions, bool validate)
{
 ...
 */
bool
_xtproperty(Oid index_oid, int attno,
 ...
 */
char *
_xtbuildphasename(int64 phasenum)
{
	switch (phasenum)
	{
...
/*
 *	_xt_truncate() -- create tuple without unneeded suffix attributes.
 *
 ...
 */
IndexTuple
_xt_truncate(Relation rel, IndexTuple lastleft, IndexTuple firstright,
			 BTScanInsert itup_key)
{
	...
	keepnatts = _xt_keep_natts(rel, lastleft, firstright, itup_key);
...
/*
 * _xt_keep_natts - how many key attributes to keep when truncating.
 *
 ...
 */
static int
_xt_keep_natts(Relation rel, IndexTuple lastleft, IndexTuple firstright,
			   BTScanInsert itup_key)
{
	...
	/*
	 * _xt_compare() treats truncated key attributes as having the value minus
	...
	/*
	 * Assert that _xt_keep_natts_fast() agrees with us in passing.  This is
	 * expected in an allequalimage index.
	 */
	Assert(!itup_key->allequalimage ||
		   keepnatts == _xt_keep_natts_fast(rel, lastleft, firstright));

	return keepnatts;
}

/*
 * _xt_keep_natts_fast - fast bitwise variant of _xt_keep_natts.
 *
 ...
 * save cycles.
 *
 * The approach taken here usually provides the same answer as _xt_keep_natts
 ...
 * _xt_keep_natts would.
 *
 * Callers can rely on the fact that attributes considered equal here are
 * definitely also equal according to _xt_keep_natts, even when the index uses
 ...
 */
int
_xt_keep_natts_fast(Relation rel, IndexTuple lastleft, IndexTuple firstright)
{
	TupleDesc	itupdesc = RelationGetDescr(rel);
...
/*
 *  _xt_check_natts() -- Verify tuple has expected number of attributes.
 *
 ...
 */
bool
_xt_check_natts(Relation rel, bool heapkeyspace, Page page, OffsetNumber offnum)
{
...
/*
 *
 *  _xt_check_third_page() -- check whether tuple fits on a btree page at all.
 *
 ...
 */
void
_xt_check_third_page(Relation rel, Relation heap, bool needheaptidspace,
 ...
 */
bool
_xt_allequalimage(Relation rel, bool debugmessage)
{

 ...
 *
 * There are rare cases where _xt_truncate() will need to enlarge
 ...
 * re-use.  However, _xt_pendingfsm_finalize() duplicates some of the same
 ...
 * tuples (non-pivot tuples).  _xt_check_natts() enforces the rules
 * described here.
 *
 ...
 *
 * See comments in _xt_first for an explanation of the nextkey and backward
 * fields.
 *
...
typedef struct BTInsertStateData
{
	 ...
	 * where _xt_check_unique is called.  See _xt_binsrch_insert and
	 * _xt_findinsertloc for details.
	 */
	bool		bounds_valid;
	OffsetNumber low;
	...
	/*
	 * if _xt_binsrch_insert found the location inside existing posting list,
 ...
 * This consists of preprocessed scan keys (see _xt_preprocess_keys() for
...
typedef struct BTScanPosData
{
	 ...
	 * We can clear the appropriate one of these flags when _xt_checkkeys()
	...
	/*
	 * Direction of the scan at the time that _xt_readpage was called.
	 *
...
{
	/* these fields are set by _xt_preprocess_keys(): */
...
/*
 * _xt_readpage state used across _xt_checkkeys calls for a page
 */
typedef struct BTReadPageState
{
	/* Input parameters, set by _xt_readpage for _xt_checkkeys */
	...
	BlockNumber prev_scan_page; /* previous _xt_parallel_release block */
	Page		page;			/* Page being read */

	/* Per-tuple input parameters, set by _xt_readpage for _xt_checkkeys */
	OffsetNumber offnum;		/* current tuple's page offset number */

	/* Output parameter, set by _xt_checkkeys for _xt_readpage */
	OffsetNumber skip;			/* Array keys "look ahead" skip offnum */
	bool		continuescan;	/* Terminate ongoing (primitive) index scan? */

	/*
	 * Input and output parameters, set and unset by both _xt_readpage and
	 * _xt_checkkeys to manage precheck optimizations
	 */
	bool		prechecked;		/* precheck set continuescan to 'true'? */
	bool		firstmatch;		/* at least one match so far?  */

	/*
	 * Private _xt_checkkeys state used to manage "look ahead" optimization
 ...
 */
extern void _xtbuildempty(Relation index);
extern bool _xtinsert(Relation rel, Datum *values, bool *isnull,
					 ItemPointer ht_ctid, Relation heapRel,
					 IndexUniqueCheck checkUnique,
					 bool indexUnchanged,
					 struct IndexInfo *indexInfo);
extern IndexScanDesc _xtbeginscan(Relation rel, int nkeys, int norderbys);
extern Size _xtestimateparallelscan(int nkeys, int norderbys);
extern void _xtinitparallelscan(void *target);
extern bool _xtgettuple(IndexScanDesc scan, ScanDirection dir);
extern int64 _xtgetbitmap(IndexScanDesc scan, TIDBitmap *tbm);
extern void _xtrescan(IndexScanDesc scan, ScanKey scankey, int nscankeys,
					 ScanKey orderbys, int norderbys);
extern void _xtparallelrescan(IndexScanDesc scan);
extern void _xtendscan(IndexScanDesc scan);
extern void _xtmarkpos(IndexScanDesc scan);
extern void _xtrestrpos(IndexScanDesc scan);
extern IndexBulkDeleteResult *_xtbulkdelete(IndexVacuumInfo *info,
										   IndexBulkDeleteResult *stats,
										   IndexBulkDeleteCallback callback,
										   void *callback_state);
extern IndexBulkDeleteResult *_xtvacuumcleanup(IndexVacuumInfo *info,
											  IndexBulkDeleteResult *stats);
extern bool _xtcanreturn(Relation index, int attno);

/*
 * prototypes for internal functions in nbtree.c
 */
extern bool _xt_parallel_seize(IndexScanDesc scan, BlockNumber *pageno,
							   bool first);
extern void _xt_parallel_release(IndexScanDesc scan, BlockNumber scan_page);
extern void _xt_parallel_done(IndexScanDesc scan);
extern void _xt_parallel_primscan_schedule(IndexScanDesc scan,
										   BlockNumber prev_scan_page);

/*
 * prototypes for functions in nbtdedup.c
 */
extern void _xt_dedup_pass(Relation rel, Buffer buf, IndexTuple newitem,
						   Size newitemsz, bool bottomupdedup);
extern bool _xt_bottomupdel_pass(Relation rel, Buffer buf, Relation heapRel,
								 Size newitemsz);
extern void _xt_dedup_start_pending(BTDedupState state, IndexTuple base,
									OffsetNumber baseoff);
extern bool _xt_dedup_save_htid(BTDedupState state, IndexTuple itup);
extern Size _xt_dedup_finish_pending(Page newpage, BTDedupState state);
extern IndexTuple _xt_form_posting(IndexTuple base, ItemPointer htids,
								   int nhtids);
extern void _xt_update_posting(BTVacuumPosting vacposting);
extern IndexTuple _xt_swap_posting(IndexTuple newitem, IndexTuple oposting,
								   int postingoff);

/*
 * prototypes for functions in nbtinsert.c
 */
extern bool _xt_doinsert(Relation rel, IndexTuple itup,
						 IndexUniqueCheck checkUnique, bool indexUnchanged,
						 Relation heapRel);
extern void _xt_finish_split(Relation rel, Relation heaprel, Buffer lbuf,
							 BTStack stack);
extern Buffer _xt_getstackbuf(Relation rel, Relation heaprel, BTStack stack,
							  BlockNumber child);

/*
 * prototypes for functions in nbtsplitloc.c
 */
extern OffsetNumber _xt_findsplitloc(Relation rel, Page origpage,
 ...
 */
extern void _xt_initmetapage(Page page, BlockNumber rootbknum, uint32 level,
							 bool allequalimage);
extern bool _xt_vacuum_needs_cleanup(Relation rel);
extern void _xt_set_cleanup_info(Relation rel, BlockNumber num_delpages);
extern void _xt_upgrademetapage(Page page);
extern Buffer _xt_getroot(Relation rel, Relation heaprel, int access);
extern Buffer _xt_gettrueroot(Relation rel);
extern int	_xt_getrootheight(Relation rel);
extern void _xt_metaversion(Relation rel, bool *heapkeyspace,
							bool *allequalimage);
extern void _xt_checkpage(Relation rel, Buffer buf);
extern Buffer _xt_getbuf(Relation rel, BlockNumber blkno, int access);
extern Buffer _xt_allocbuf(Relation rel, Relation heaprel);
extern Buffer _xt_relandgetbuf(Relation rel, Buffer obuf,
							   BlockNumber blkno, int access);
extern void _xt_relbuf(Relation rel, Buffer buf);
extern void _xt_lockbuf(Relation rel, Buffer buf, int access);
extern void _xt_unlockbuf(Relation rel, Buffer buf);
extern bool _xt_conditionallockbuf(Relation rel, Buffer buf);
extern void _xt_upgradelockbufcleanup(Relation rel, Buffer buf);
extern void _xt_pageinit(Page page, Size size);
extern void _xt_delitems_vacuum(Relation rel, Buffer buf,
								OffsetNumber *deletable, int ndeletable,
								BTVacuumPosting *updatable, int nupdatable);
extern void _xt_delitems_delete_check(Relation rel, Buffer buf,
									  Relation heapRel,
									  TM_IndexDeleteOp *delstate);
extern void _xt_pagedel(Relation rel, Buffer leafbuf, BTVacState *vstate);
extern void _xt_pendingfsm_init(Relation rel, BTVacState *vstate,
								bool cleanuponly);
extern void _xt_pendingfsm_finalize(Relation rel, BTVacState *vstate);

/*
 * prototypes for functions in nbtsearch.c
 */
extern BTStack _xt_search(Relation rel, Relation heaprel, BTScanInsert key,
						  Buffer *bufP, int access);
extern Buffer _xt_moveright(Relation rel, Relation heaprel, BTScanInsert key,
							Buffer buf, bool forupdate, BTStack stack,
							int access);
extern OffsetNumber _xt_binsrch_insert(Relation rel, BTInsertState insertstate);
extern int32 _xt_compare(Relation rel, BTScanInsert key, Page page, OffsetNumber offnum);
extern bool _xt_first(IndexScanDesc scan, ScanDirection dir);
extern bool _xt_next(IndexScanDesc scan, ScanDirection dir);
extern Buffer _xt_get_endpoint(Relation rel, uint32 level, bool rightmost);

/*
 * prototypes for functions in nbtutils.c
 */
extern BTScanInsert _xt_mkscankey(Relation rel, IndexTuple itup);
extern void _xt_freestack(BTStack stack);
extern bool _xt_start_prim_scan(IndexScanDesc scan, ScanDirection dir);
extern void _xt_start_array_keys(IndexScanDesc scan, ScanDirection dir);
extern void _xt_preprocess_keys(IndexScanDesc scan);
extern bool _xt_checkkeys(IndexScanDesc scan, BTReadPageState *pstate, bool arrayKeys,
						  IndexTuple tuple, int tupnatts);
extern void _xt_killitems(IndexScanDesc scan);
extern BTCycleId _xt_vacuum_cycleid(Relation rel);
extern BTCycleId _xt_start_vacuum(Relation rel);
extern void _xt_end_vacuum(Relation rel);
extern void _xt_end_vacuum_callback(int code, Datum arg);
extern Size BTreeShmemSize(void);
extern void BTreeShmemInit(void);
extern bytea *_xtoptions(Datum reloptions, bool validate);
extern bool _xtproperty(Oid index_oid, int attno,
					   IndexAMProperty prop, const char *propname,
					   bool *res, bool *isnull);
extern char *_xtbuildphasename(int64 phasenum);
extern IndexTuple _xt_truncate(Relation rel, IndexTuple lastleft,
							   IndexTuple firstright, BTScanInsert itup_key);
extern int	_xt_keep_natts_fast(Relation rel, IndexTuple lastleft,
								IndexTuple firstright);
extern bool _xt_check_natts(Relation rel, bool heapkeyspace, Page page,
							OffsetNumber offnum);
extern void _xt_check_third_page(Relation rel, Relation heap,
								 bool needheaptidspace, Page page, IndexTuple newtup);
extern bool _xt_allequalimage(Relation rel, bool debugmessage);

/*
 * prototypes for functions in nbtvalidate.c
 */
extern bool _xtvalidate(Oid opclassoid);
extern void _xtadjustmembers(Oid opfamilyoid,
							Oid opclassoid,
							List *operators,
							List *functions);
 ...
 */
extern IndexBuildResult *_xtbuild(Relation heap, Relation index,
								 struct IndexInfo *indexInfo);
extern void _xt_parallel_build_main(dsm_segment *seg, shm_toc *toc);

...
static void _xt_drop_lock_and_maybe_pin(IndexScanDesc scan, BTScanPos sp);
static OffsetNumber _xt_binsrch(Relation rel, BTScanInsert key, Buffer buf);
static int	_xt_binsrch_posting(BTScanInsert key, Page page,
								OffsetNumber offnum);
static bool _xt_readpage(IndexScanDesc scan, ScanDirection dir,
						 OffsetNumber offnum, bool firstPage);
static void _xt_saveitem(BTScanOpaque so, int itemIndex,
						 OffsetNumber offnum, IndexTuple itup);
static int	_xt_setuppostingitems(BTScanOpaque so, int itemIndex,
								  OffsetNumber offnum, ItemPointer heapTid,
								  IndexTuple itup);
static inline void _xt_savepostingitem(BTScanOpaque so, int itemIndex,
									   OffsetNumber offnum,
									   ItemPointer heapTid, int tupleOffset);
static bool _xt_steppage(IndexScanDesc scan, ScanDirection dir);
static bool _xt_readnextpage(IndexScanDesc scan, BlockNumber blkno, ScanDirection dir);
static bool _xt_parallel_readpage(IndexScanDesc scan, BlockNumber blkno,
								  ScanDirection dir);
static Buffer _xt_walk_left(Relation rel, Buffer buf);
static bool _xt_endpoint(IndexScanDesc scan, ScanDirection dir);
static inline void _xt_initialize_more_data(BTScanOpaque so, ScanDirection dir);


/*
 *	_xt_drop_lock_and_maybe_pin()
 *
 ...
 */
static void
_xt_drop_lock_and_maybe_pin(IndexScanDesc scan, BTScanPos sp)
{
	_xt_unlockbuf(scan->indexRelation, sp->buf);
...
/*
 *	_xt_search() -- Search the tree for a particular scankey,
 ...
 * might need to allocate a new root page for caller -- see _xt_allocbuf.
 */
BTStack
_xt_search(Relation rel, Relation heaprel, BTScanInsert key, Buffer *bufP,
		   int access)
{
	BTStack		stack_in = NULL;
	int			page_access = BT_READ;

	/* heaprel must be set whenever _xt_allocbuf is reachable */
	Assert(access == BT_READ || access == BT_WRITE);
	Assert(access == BT_READ || heaprel != NULL);

	/* Get the root page to start with */
	*bufP = _xt_getroot(rel, heaprel, access);
	...
	for (;;)
	{
		 ...
		 *
		 * In write-mode, allow _xt_moveright to finish any incomplete splits
		 * along the way.  Strictly speaking, we'd only need to finish an
		 * incomplete split on the leaf page we're about to insert to, not on
		 * any of the upper levels (internal pages with incomplete splits are
		 * also taken care of in _xt_getstackbuf).  But this is a good
		 * opportunity to finish splits of internal pages too.
		 */
		*bufP = _xt_moveright(rel, heaprel, key, *bufP, (access == BT_WRITE),
		 ...
		 */
		offnum = _xt_binsrch(rel, key, *bufP);
		...
		*bufP = _xt_relandgetbuf(rel, *bufP, child, page_access);
	...
	if (access == BT_WRITE && page_access == BT_READ)
	{
		...
		_xt_unlockbuf(rel, *bufP);
		_xt_lockbuf(rel, *bufP, BT_WRITE);
		 ...
		 */
		*bufP = _xt_moveright(rel, heaprel, key, *bufP, true, stack_in, BT_WRITE);
	}
...
/*
 *	_xt_moveright() -- move right in the btree if necessary.
 *
			  ...
			  int access)
{
	 ...
	 * _xt_search has found an initial leaf page.)
	 *
	...
	for (;;)
	{
		...
		if (forupdate && P_INCOMPLETE_SPLIT(opaque))
		{
			...
			if (P_INCOMPLETE_SPLIT(opaque))
				_xt_finish_split(rel, heaprel, buf, stack);
			else
				_xt_relbuf(rel, buf);

			/* re-acquire the lock in the right mode, and re-check */
			buf = _xt_getbuf(rel, blkno, access);
			continue;
		}

		if (P_IGNORE(opaque) || _xt_compare(rel, key, page, P_HIKEY) >= cmpval)
		{
			/* step right one page */
			buf = _xt_relandgetbuf(rel, buf, opaque->btpo_next, access);
			continue;
		}
		else
			break;
...
/*
 *	_xt_binsrch() -- Do a binary search for a key on a particular page.
 *
 * On an internal (non-leaf) page, _xt_binsrch() returns the OffsetNumber
 * of the last key < given scankey, or last key <= given scankey if nextkey
 * is true.  (Since _xt_compare treats the first data key of such a page as
 * minus infinity, there will be at least one key < scankey, so the result
 * always points at one of the keys on the page.)
 *
 * On a leaf page, _xt_binsrch() returns the final result of the initial
 * positioning process that started with _xt_first's call to _xt_search.
 ...
 * the given page.  _xt_binsrch() has no lock or refcount side effects
 * on the buffer.
 */
static OffsetNumber
_xt_binsrch(Relation rel,
			BTScanInsert key,
			Buffer buf)
{
	Page		page;
	...
	/* scantid-set callers must use _xt_binsrch_insert() on leaf pages */
	...
	while (high > low)
	{
		...
		result = _xt_compare(rel, key, page, mid);
	 ...
	 * There must be one if _xt_compare() is playing by the rules.
	 *
	 * _xt_compare() will seldom see any exactly-matching pivot tuples, since
	 ...
	 *
	 * However, during backward scans _xt_compare() interprets omitted scan
...
/*
 *
 *	_xt_binsrch_insert() -- Cacheable, incremental leaf page binary search.
 *
 * Like _xt_binsrch(), but with support for caching the binary search
 ...
 */
OffsetNumber
_xt_binsrch_insert(Relation rel, BTInsertState insertstate)
{
	...
	while (high > low)
	{
		...
		if (unlikely(result == 0 && key->scantid != NULL))
		{
			...
			insertstate->postingoff = _xt_binsrch_posting(key, page, mid);
		}
	}
...
/*----------
 *	_xt_binsrch_posting() -- posting list binary search.
 *
 * Helper routine for _xt_binsrch_insert().
 *
 * Returns offset into posting list where caller's scantid belongs.
 *----------
 */
static int
_xt_binsrch_posting(BTScanInsert key, Page page, OffsetNumber offnum)
{
	IndexTuple	itup;
	ItemId		itemid;
	int			low,
	 ...
	 * to be able to relocate a non-pivot tuple using _xt_binsrch_insert().)
	 */
	itemid = PageGetItemId(page, offnum);
	itup = (IndexTuple) PageGetItem(page, itemid);
	 ...
	 * to _xt_binsrch_insert() caller by returning -1, a sentinel value.  A
	 * second call to _xt_binsrch_insert() can take place when its caller has
...
/*----------
 *	_xt_compare() -- Compare insertion-type scankey to tuple on a page.
 *
			...
			OffsetNumber offnum)
{
	...
	Assert(_xt_check_natts(rel, key->heapkeyspace, page, offnum));
	...
	ntupatts = 1;//BTreeTupleGetNAtts(itup, rel);
	...
	ncmpkey = 1;//Min(ntupatts, key->keysz);
	...
	//for (int i = 1; i <= ncmpkey; i++)
	{
		...
		// else
		{
			 ...
			 * _xt_compare as comparing the scankey to the index item, we have
	...
	if (key->scantid == NULL)
	{
		 ...
		 * (See _xt_binsrch for an explanation of our backward scan behavior.)
		 *
...
/*
 *	_xt_first() -- Find the first item in a scan.
 *
 ...
 */
bool
_xt_first(IndexScanDesc scan, ScanDirection dir)
{
	 ...
	 */
	_xt_preprocess_keys(scan);

	/*
	 * Quit now if _xt_preprocess_keys() discovered that the scan keys can
	...
	{
		status = _xt_parallel_seize(scan, &blkno, true);

		/*
		 * Initialize arrays (when _xt_parallel_seize didn't already set up
		...
		{
			if (!_xt_parallel_readpage(scan, blkno, dir))
				return false;
			goto readcomplete;
		}
	}
	...
	{
		/*
		 * First _xt_first call (for current btrescan) without parallelism.
		 *
		 * Initialize arrays, and the corresponding scan keys that were just
		 * output by _xt_preprocess_keys.
		 */
		_xt_start_array_keys(scan, dir);
	}
	 ...
	 *
	 * When the scan keys include cross-type operators, _xt_preprocess_keys
	 ...
	 * _xt_checkkeys() will stop the scan as soon as an equality qual fails.
	 ...
	 *
	 * _xt_checkkeys/_xt_advance_array_keys decide whether and when to start
	 ...
	 * If you update anything here, _xt_checkkeys/_xt_advance_array_keys might
	...
	for (i = 0; i < keysz; i++)
	{
		...
		if (cur->sk_flags & SK_ROW_HEADER)
		{
			 ...
			 * out for nulls, which _xt_preprocess_keys didn't check. A null
	 ...
	 * initial descent by _xt_search (and our _xt_binsrch call for the leaf
	 * page _xt_search returns).
	 *----------
	 */
	_xt_metaversion(rel, &inskey.heapkeyspace, &inskey.allequalimage);
	...
	stack = _xt_search(rel, NULL, &inskey, &buf, BT_READ);
	...
	if (!BufferIsValid(buf))
	{
		 ...
		 * _xt_search() and PredicateLockRelation().  We have to try again
	...
	_xt_initialize_more_data(so, dir);

	/* position to the precise item on the page */
	offnum = _xt_binsrch(rel, &inskey, buf);
	Assert(!BTScanPosIsValid(so->currPos));
	so->currPos.buf = buf;
	 ...
	 * positioned at the last non-pivot tuple <= inskey.scankeys.
	 *
	 * It's possible that _xt_binsrch returned an offnum that is out of bounds
	 ...
	 */
	if (!_xt_readpage(scan, dir, offnum, true))
	{
		/*
		...
		if (!_xt_steppage(scan, dir))
			return false;
	}
	else
	{
		/* We have at least one item to return as scan's first item */
		_xt_drop_lock_and_maybe_pin(scan, &so->currPos);
	}
...
/*
 *	_xt_next() -- Get the next item in a scan.
 *
 ...
 */
bool
_xt_next(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	BTScanPosItem *currItem;
...
/*
 *	_xt_readpage() -- Load data from current index page into so->currPos
 *
 ...
 * moreLeft or moreRight (as appropriate) is cleared if _xt_checkkeys reports
 * that there can be no more matching tuples in the current scan direction
 * (could just be for the current primitive index scan when scan has arrays).
 *
 * _xt_first caller passes us an offnum returned by _xt_binsrch, which might
 * be an out of bounds offnum such as "maxoff + 1" in certain corner cases.
 * _xt_checkkeys will stop the scan as soon as an equality qual fails (when
 * its scan key was marked required), so _xt_first _must_ pass us an offnum
 ...
 * the scan on this page by calling _xt_checkkeys against the high key.
 *
 * In the case of a parallel scan, caller must have called _xt_parallel_seize
 * prior to calling this function; this function will invoke
 * _xt_parallel_release before returning.
 *
 * Returns true if any matching items found on the page, false if none.
 */
static bool
_xt_readpage(IndexScanDesc scan, ScanDirection dir, OffsetNumber offnum,
			 bool firstPage)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	Page		page;
	...
	if (scan->parallel_scan)
	{
		...
		_xt_parallel_release(scan, pstate.prev_scan_page);
	}
	...
	/* initialize page-level state that we'll pass to _xt_checkkeys */
	pstate.dir = dir;
	 ...
	 * allows us to do significant optimizations in the _xt_checkkeys()
	 ...
	 *
	 * The optimization is unsafe and must be avoided whenever _xt_checkkeys
	...
	if (!firstPage && !so->scanBehind && minoff < maxoff)
	{
		...
		_xt_checkkeys(scan, &pstate, false, itup, indnatts);
	...
	if (ScanDirectionIsForward(dir))
	{
		...
		while (offnum <= maxoff)
		{
			...
			passes_quals = _xt_checkkeys(scan, &pstate, arrayKeys,
										 itup, indnatts);
			...
			if (passes_quals)
			{
				...
				{
					/* Remember it */
					_xt_saveitem(so, itemIndex, offnum, itup);
					itemIndex++;
				}
				else
				{
					 ...
					 */
					tupleOffset =
						_xt_setuppostingitems(so, itemIndex, offnum,
					...
					{
						_xt_savepostingitem(so, itemIndex, offnum,
		...
		if (pstate.continuescan && !P_RIGHTMOST(opaque))
		{
			...
			_xt_checkkeys(scan, &pstate, arrayKeys, itup, truncatt);
		}
		...
		while (offnum >= minoff)
		{
			...
			if (passes_quals && tuple_alive)
			{
				...
				else
				{
					 ...
					 * scans.  This allows _xt_killitems() to make a
...
static void
_xt_saveitem(BTScanOpaque so, int itemIndex,
			 OffsetNumber offnum, IndexTuple itup)
{
 ...
 * be saved by calling _xt_savepostingitem().
 *
 * Returns an offset into tuple storage space that main tuple is stored at if
 * needed.
 */
static int
_xt_setuppostingitems(BTScanOpaque so, int itemIndex, OffsetNumber offnum,
 ...
 * tuple.
 *
 * Assumes that _xt_setuppostingitems() has already been called for current
 * posting list tuple.  Caller passes its return value as tupleOffset.
 */
static inline void
_xt_savepostingitem(BTScanOpaque so, int itemIndex, OffsetNumber offnum,
					ItemPointer heapTid, int tupleOffset)
{
	BTScanPosItem *currItem = &so->currPos.items[itemIndex];
...
/*
 *	_xt_steppage() -- Step to next page containing valid data for scan
 *
 ...
 */
static bool
_xt_steppage(IndexScanDesc scan, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	...
	if (so->markItemIndex >= 0)
	{
		 ...
		 * _xt_readpage call (that takes place after it has restored markPos).
		 */
	...
	if (ScanDirectionIsForward(dir))
	{
		...
		if (scan->parallel_scan != NULL)
		{
			 ...
			 */
			status = _xt_parallel_seize(scan, &blkno, false);
			if (!status)
			{
	...
	if (!_xt_readnextpage(scan, blkno, dir))
		return false;
...
/*
 *	_xt_readnextpage() -- Read next page containing valid data for scan
 *
 ...
 */
static bool
_xt_readnextpage(IndexScanDesc scan, BlockNumber blkno, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	Relation	rel;
	Page		page;
	...
	if (ScanDirectionIsForward(dir))
	{
		for (;;)
		{
			...
			/* step right one page */
			so->currPos.buf = _xt_getbuf(rel, blkno, BT_READ);
			page = BufferGetPage(so->currPos.buf);
			opaque = BTPageGetOpaque(page);
			...
			if (!P_IGNORE(opaque))
			{
				...
				if (_xt_readpage(scan, dir, P_FIRSTDATAKEY(opaque), false))
					break;
			}
			else if (scan->parallel_scan != NULL)
			{
				/* allow next page be processed by parallel worker */
				_xt_parallel_release(scan, opaque->btpo_next);
			}

			/* nope, keep going */
			if (scan->parallel_scan != NULL)
			{
				_xt_relbuf(rel, so->currPos.buf);
				status = _xt_parallel_seize(scan, &blkno, false);
				if (!status)
				{
					BTScanPosInvalidate(so->currPos);
	...
	else
	{
		 ...
		 */
		if (BTScanPosIsPinned(so->currPos))
			_xt_lockbuf(rel, so->currPos.buf, BT_READ);
		else
			so->currPos.buf = _xt_getbuf(rel, so->currPos.currPage, BT_READ);
		...
		for (;;)
		{
			...
			so->currPos.buf = _xt_walk_left(rel, so->currPos.buf);
			...
			if (!P_IGNORE(opaque))
			{
				...
				if (_xt_readpage(scan, dir, PageGetMaxOffsetNumber(page), false))
					break;
			}
			else if (scan->parallel_scan != NULL)
			{
				/* allow next page be processed by parallel worker */
				_xt_parallel_release(scan, BufferGetBlockNumber(so->currPos.buf));
			}
...
/*
 *	_xt_parallel_readpage() -- Read current page containing valid data for scan
 *
 * On success, release lock and maybe pin on buffer.  We return true to
 * indicate success.
 */
static bool
_xt_parallel_readpage(IndexScanDesc scan, BlockNumber blkno, ScanDirection dir)
{
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
...
/*
 * _xt_walk_left() -- step left one page, if possible
 *
 ...
 */
static Buffer
_xt_walk_left(Relation rel, Buffer buf)
{
	Page		page;
	BTPageOpaque opaque;
	...
	for (;;)
	{
		...
		/* check for interrupts while we're not holding any buffer lock */
		CHECK_FOR_INTERRUPTS();
		buf = _xt_getbuf(rel, blkno, BT_READ);
		page = BufferGetPage(buf);
		opaque = BTPageGetOpaque(page);
		...
		for (;;)
		{
			...
			if (P_RIGHTMOST(opaque) || ++tries > 4)
				break;
			blkno = opaque->btpo_next;
			buf = _xt_relandgetbuf(rel, buf, blkno, BT_READ);
			page = BufferGetPage(buf);
			opaque = BTPageGetOpaque(page);
		}

		/* Return to the original page to see what's up */
		buf = _xt_relandgetbuf(rel, buf, obknum, BT_READ);
		page = BufferGetPage(buf);
		opaque = BTPageGetOpaque(page);
...
/*
 * _xt_get_endpoint() -- Find the first or last page on a given tree level
 *
 ...
 */
Buffer
_xt_get_endpoint(Relation rel, uint32 level, bool rightmost)
{
	Buffer		buf;
	Page		page;
	BTPageOpaque opaque;
	 ...
	 */
	if (level == 0)
		buf = _xt_getroot(rel, NULL, BT_READ);
	else
		buf = _xt_gettrueroot(rel);
...
/*
 *	_xt_endpoint() -- Find the first or last page in the index, and scan
 * from there to the first key satisfying all the quals.
 *
 * This is used by _xt_first() to set up a scan when we've determined
 * that the scan must start at the beginning or end of the index (for
 * a forward or backward scan respectively).  Exit conditions are the
 * same as for _xt_first().
 */
static bool
_xt_endpoint(IndexScanDesc scan, ScanDirection dir)
{
	Relation	rel = scan->indexRelation;
	BTScanOpaque so = (BTScanOpaque) scan->opaque;
	Buffer		buf;
	 ...
	 * version of _xt_search().  We don't maintain a stack since we know we
	 * won't need it.
	 */
	buf = _xt_get_endpoint(rel, 0, ScanDirectionIsBackward(dir));
	 ...
	 * Now load data from the first page of the scan.
	 */
	if (!_xt_readpage(scan, dir, start, true))
	{
		/*
		 * There's no actually-matching data on this page.  Try to advance to
		 * the next page.  Return false if there's no matching data at all.
...
/*
 * _xt_initialize_more_data() -- initialize moreLeft, moreRight and scan dir
 * from currPos
 */
static inline void
_xt_initialize_more_data(BTScanOpaque so, ScanDirection dir)
{
	so->currPos.dir = dir;
	if (so->needPrimScan)
	{
